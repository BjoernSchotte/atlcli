---
title: "Getting Started"
description: "Install atlcli, authenticate with Atlassian, and run your first commands"
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

# Getting Started

This guide walks you through installing atlcli, authenticating with Atlassian, and running your first commands.

## Installation

### Quick Install (Recommended)

The fastest way to install atlcli on macOS or Linux:

```bash
curl -fsSL https://atlcli.sh/install.sh | bash
```

Install a specific version:

```bash
curl -fsSL https://atlcli.sh/install.sh | bash -s v0.16.0
```

This installs to `~/.atlcli/bin` and updates your shell PATH automatically.

:::note[Verified downloads]
The install script downloads the release archive, verifies its SHA-256 checksum against the `checksums.txt` published with each release, and only then installs. If the checksums file is unavailable — for example, for a pre-release — it warns and continues.
:::

### Homebrew

Alternatively, install via Homebrew:

```bash
brew install bjoernschotte/tap/atlcli
```

To upgrade:

```bash
brew update && brew upgrade atlcli
```

To preview a build from a green `main` commit, switch to the separate dev
formula:

```bash
brew uninstall atlcli
brew install bjoernschotte/tap/atlcli-dev
atlcli release-info --json
```

The two formulae conflict intentionally and cannot be linked together. See
[Development releases](/reference/dev-releases/) for upgrade and return-to-stable
instructions.

### Manual Download

Download the latest release for your platform from the [releases page](https://github.com/BjoernSchotte/atlcli/releases):

| Platform | File |
|----------|------|
| macOS (Apple Silicon) | `atlcli-darwin-arm64.tar.gz` |
| macOS (Intel) | `atlcli-darwin-x64.tar.gz` |
| Linux (ARM64) | `atlcli-linux-arm64.tar.gz` |
| Linux (x64) | `atlcli-linux-x64.tar.gz` |
| Windows (x64) | `atlcli-windows-x64.zip` |

On macOS/Linux, extract and add to your PATH:

```bash
tar -xzf atlcli-*.tar.gz
sudo mv atlcli /usr/local/bin/
```

On Windows, extract the `.zip` and add the folder containing `atlcli.exe` to your `PATH`.

:::note[Windows SmartScreen]
The Windows binary is unsigned, so SmartScreen may show a warning on first run. Click **More info** → **Run anyway** to continue. Signed releases are not currently provided.
:::

:::note[macOS Gatekeeper]
The macOS binaries are not notarized. On first run you may need to remove the quarantine attribute:
```bash
xattr -d com.apple.quarantine /usr/local/bin/atlcli
```
:::

### From Source

For development or if you want the latest unreleased changes:

**Prerequisites:**

- [Bun](https://bun.sh) v1.0 or later

```bash
git clone https://github.com/BjoernSchotte/atlcli.git
cd atlcli
bun install
bun run build
```

The CLI is now available at `./apps/cli/dist/atlcli`.

:::tip[Add to PATH]
Add the dist directory to your PATH or create an alias:
```bash
alias atlcli="$(pwd)/apps/cli/dist/atlcli"
```
:::

## Shell Completion

Enable tab completion for commands and flags:

<Tabs>
  <TabItem label="Zsh">
    ```bash
    # Add to ~/.zshrc
    eval "$(atlcli completion zsh)"
    ```
  </TabItem>
  <TabItem label="Bash">
    ```bash
    # Add to ~/.bashrc
    eval "$(atlcli completion bash)"
    ```
  </TabItem>
</Tabs>

Restart your shell or run `source ~/.zshrc` (or `~/.bashrc`) to activate.

[Full Shell Completion Guide](/reference/shell-completions/)

## Authentication

atlcli supports both Atlassian Cloud and Server/Data Center instances. You can manage multiple profiles for different instances.

### Cloud Instances

For Cloud (`*.atlassian.net`), you need an API token:

1. Go to [Atlassian Account Settings](https://id.atlassian.com/manage-profile/security/api-tokens)
2. Click **Create API token**
3. Give it a descriptive name (e.g., "atlcli")
4. Copy the token

### Server/Data Center Instances

For Server or Data Center, create a Personal Access Token (PAT) in your instance's user settings, then use `--bearer` when logging in:

```bash
atlcli auth login --bearer --site https://jira.company.com --token YOUR_PAT
```

If your instance uses a self-signed certificate or an internal Certificate Authority, pass `--ca-file` to point atlcli at your CA bundle:

```bash
atlcli auth login --bearer --site https://jira.company.internal \
  --token YOUR_PAT --ca-file /etc/ssl/certs/company-ca.pem
```

See [Authentication](/authentication/) for details on token storage, Mac Keychain integration, and [TLS / self-signed certificates](/authentication/#tls-and-self-signed-certificates).

### Initialize Authentication

```bash
atlcli auth init
```

Follow the prompts to enter:

- **Instance URL**: Your Atlassian instance (e.g., `https://yourcompany.atlassian.net`)
- **Email**: Your Atlassian account email
- **API Token**: The token you created

Your credentials are stored securely at `~/.atlcli/credentials.json`.

### Multiple Profiles

Create named profiles for different instances:

```bash
atlcli auth init --profile work
atlcli auth init --profile personal
```

Use `--profile` with any command:

```bash
atlcli jira search --assignee me --profile work
```

## Confluence Quick Start

### Initialize a Local Directory

```bash
atlcli wiki docs init ./team-docs --space TEAM
```

This creates a local directory linked to the TEAM space.

### Pull Pages

```bash
atlcli wiki docs pull ./team-docs
```

Pages are downloaded as markdown files with YAML frontmatter:

```markdown
---
id: "12345"
title: "Meeting Notes"
space: "TEAM"
---

# Meeting Notes

Content here...
```

### Edit and Push

Edit files locally with your favorite editor, then push changes:

```bash
atlcli wiki docs push ./team-docs
```

atlcli detects changes and updates only modified pages.

## Jira Quick Start

### Search Issues

```bash
# Your assigned issues
atlcli jira search --assignee me

# Open bugs in a project
atlcli jira search --project PROJ --type Bug --status Open

# Using JQL directly
atlcli jira search --jql "project = PROJ AND sprint in openSprints()"
```

### View an Issue

```bash
atlcli jira issue get --key PROJ-123
```

### Create an Issue

```bash
atlcli jira issue create --project PROJ --type Task --summary "Fix login bug"
```

### Track Time

```bash
# Start a timer
atlcli jira worklog timer start PROJ-123

# Stop and log time
atlcli jira worklog timer stop PROJ-123

# Or log directly
atlcli jira worklog add PROJ-123 --time 2h --comment "Code review"
```

## JSON Output

All commands support `--json` for scripting:

<Tabs>
  <TabItem label="Human-readable">
    ```bash
    atlcli jira search --assignee me
    ```
    ```
    PROJ-123  In Progress  Fix login bug
    PROJ-124  To Do        Add dark mode
    ```
  </TabItem>
  <TabItem label="JSON">
    ```bash
    atlcli jira search --assignee me --json
    ```
    ```json
    {
      "schemaVersion": "1",
      "issues": [
        {"key": "PROJ-123", "status": "In Progress", "summary": "Fix login bug"},
        {"key": "PROJ-124", "status": "To Do", "summary": "Add dark mode"}
      ]
    }
    ```
  </TabItem>
</Tabs>

## Next Steps

- [Confluence Guide](/confluence/) - Deep dive into sync, templates, macros
- [Jira Guide](/jira/) - Issues, boards, sprints, analytics
- [Recipes](/recipes/) - Real-world workflows
- [Configuration](/configuration/) - Customize atlcli behavior
