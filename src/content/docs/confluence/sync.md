...
Enable the bundled Draw.io plugin and install Draw.io Desktop with its CLI on your `PATH`:

```bash
atlcli plugin enable drawio
atlcli drawio preview ./docs
atlcli drawio watch ./docs
```

Pulled Draw.io macros become a local preview image beside their editable
`<diagram>.drawio` source. Edit only the `.drawio` file. The paired
`.drawio.png` file is generated by Draw.io Desktop for VS Code Markdown Preview
and Confluence; do not edit it manually.

With the plugin enabled, `docs push` regenerates stale previews before it uploads
the source and preview attachment. Use `--executable <path>` with `drawio preview`
