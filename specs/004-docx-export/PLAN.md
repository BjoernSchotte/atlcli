...
- [x] Callout boxes as styled 1×1 tables (background + left accent border + title); source table-cell backgrounds render through `w:shd` with readable light/dark ink and continue across vertical merges; code blocks via Shiki colored runs — verified by unzipping the output in tests and asserting OOXML landmarks — `ooxml.ts` `calloutTable`/`tableCell`/`codeLineParagraph`, `highlight.ts` (lazy fine-grained Shiki core, JS regex engine, curated per-language chunks); `serialize.test.ts` asserts fills, merge continuation, contrast ink, accents, and colored runs
