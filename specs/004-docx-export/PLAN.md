...
prototyped in `image-module-prototype.ts`) becomes its own follow-up task **after** this spec's
...
- [x] Self-built OOXML image module effort estimated (~1 day) + prototyped in `image-module-prototype.ts` — recorded as a cost item; also surfaced the decisive MV3-CSP/`eval` finding that rules out docx-templates
...
- **F3 — image embedding**: ✅ (Björn, 2026-07-16) **deferred to a follow-up task**. v1 export omits images with a report line; the ~1-day self-built OOXML image module (prototyped in `image-module-prototype.ts`) is a separate task after the export flow is proven.
