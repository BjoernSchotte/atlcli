...
- **Active-page detection across worker restarts and windows** (fixed 2026-07-17): The MV3 worker previously kept its monotonic detection sequence only in a global variable. When Chrome stopped and restarted the worker, the sequence reset to 0 while an open side panel retained a higher lastSeq, so subsequent SPA navigations and tab switches were rejected as stale and exports kept using the old page. The pull path also searched active tabs across all windows and could select a page from the wrong window. The observer cursor now lives in chrome.storage.session, all pull and push transitions are serialized, deduplication is tracked per window, the panel sends its concrete windowId, and broadcasts are filtered to that window. Regression coverage includes cursor hydration after restart, async pull-versus-push ordering, same-URL events in two windows, exact window routing, and invalid message IDs.
