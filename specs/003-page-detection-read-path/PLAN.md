...
- **E2E papercut — detection lost after an extension reload without F5** (live E2E, 2026-07-14): after reloading the unpacked extension, the panel showed "No Atlassian page detected" for an already-open Confluence tab until the user pressed F5. Root cause: the SW restarts and the panel remounts and sends `get-current-entity`, but the already-open tab fires no `tabs.onActivated`/`onUpdated`, so the mount pull is the only detection path — and `getCurrentEntity` read the wrong tab. `chrome.tabs.query({ active: true, lastFocusedWindow: true })` can resolve to the wrong tab when the panel has focus (the extractor itself was confirmed correct on `/wiki/spaces/RCM/pages/1031503874/…`; this is purely which tab URL the SW reads). **Fix (two parts):** (1) `background.ts getCurrentEntity` now queries the last-focused active tab AND every window's active tab (`Promise.all` of the two `chrome.tabs.query` calls) and passes both to a new pure selector `selectActiveTabUrl` (in `utils/tab-observer.ts`) that prefers the docked Atlassian tab — last-focused-if-Atlassian → any active Atlassian tab → last-focused → first active tab; (2) `App.tsx` re-pulls (`get-current-entity`) on `document.visibilitychange`/`window.focus` so re-opening or refocusing the panel recovers detection with no page reload. The pure `observeTab`/`currentDetection`/reducer logic is unchanged; the reducer's `seq` ordering guard still prevents a stale pull from overwriting a newer push. Impl: `apps/extension/utils/tab-observer.ts` (`selectActiveTabUrl`), `apps/extension/entrypoints/background.ts` (widened query), `apps/extension/utils/detection-pull.ts` + `entrypoints/sidepanel/App.tsx` (`pullCurrentEntity` + visibility/focus listeners); regression tests in `tests/tab-observer.test.ts` (selector: docked-tab recovery, foreign-origin non-preference, fallbacks) and `tests/detection-pull.test.ts` (pull dispatches `detected` with url/entity/seq; re-pull latest seq; asleep-SW/unrelated-response/undefined → no dispatch). **Live-only:** whether `lastFocusedWindow` actually mis-resolves after a reload (and thus whether the widened query fully closes the papercut) can only be confirmed in a real browser — the selector logic is unit-tested, the chrome-query change itself is left for the E2E re-test.
