...
read/write objective remains open. Implementation evidence reviewed through Slice 114; broad build/CI evidence below
...
| CREATE/RENAME/REMOVE and editor replacement saves | Slices 72–89 implement journaled local files/directories, CREATE modes, metadata, backup rename and replacement under original page IDs; Slice 103 automatically creates plain Markdown through native Vim on both OSes, with Linux DOCSY verification | Slice 111 covers native WebDAV Vim new pages on both OSes; ambiguous creation reconciliation, Linux VS Code new-page tests (Slice 113 covers native macOS VS Code new pages/autosave; Slice 112 covers native macOS TextEdit new pages on both transports), tree rename and directory removal remain; Slice 108 adds guarded page-body trash, and Slice 109 confirms explicit remote trash after restart; other uncertain outcomes remain unresolved; full overwritten/unlinked handle lifetime and mutation-race audit |
