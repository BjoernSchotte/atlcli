...
Status: **proposed for review; no runtime implementation has started**
...
- DeepAgentsJS documents dynamic subagents as runtime-selected specialists, but
  the host still owns their declared capabilities and admission policy:
...
The installed cross-host runtime is DeepAgentsJS `1.12.1`. Its browser export
...
typed subagent event streams. The current repository already has the safer
building blocks needed here in:
...
### Recommended conversational deadlines

These are defaults, not semantic limitations on workflow shape:

- Auto Chat: 60-second soft deadline, 120-second default hard deadline.
- Deep Chat: 90-second soft deadline, 180-second default hard deadline.
- Reserve at least the final 20% of remaining time for validation and synthesis.
- Deep Research retains its separate, configurable long-run budget (currently up
  to ten minutes by default).
...
- The evidence critic is independent of the acquisition trajectory. It receives
  accepted evidence/claim projections and checks coverage, contradiction,
  citation support, task completion, and source suitability.
...
- If an uninterruptible provider/tool call is already in flight, its eventual
  result is marked superseded and quarantined rather than admitted into the new
  revision. Steering never splices arbitrary text into a model's hidden
  generation state.
- A stop request aborts supervisor model calls, child model calls, QuickJS,
  capability calls, pagination, and queued frontier dispatch through one shared
  `AbortSignal`/host cancellation boundary.
...
- Each child has durable `queued`, `running`, `succeeded`, `failed`,
  `cancelled`, `superseded`, or `unknown` state. A crash after an external call
  but before result commit becomes `unknown`; it is never blindly repeated.
...
Each row has stable IDs and lifecycle state (`queued`, `running`, `completed`,
`failed`, `superseded`). Only the active row carries the animated Kite mark.
...
### T0 — Review and freeze the behavior contract

- [ ] Review and accept the mode matrix and Chat/Deep Research boundary.
- [ ] Confirm recommended Auto/Deep soft and hard deadline defaults.
...
- [ ] Add the accepted contract to the main issue-138 plan without duplicating
      implementation tasks.

Proof: plan review only; no runtime changes.
...
Proof: contract, persistence migration, provider-adapter, CLI parse, and UI tests.

### T2 — Generalize the existing dynamic workflow engine

- [ ] Extract graph admission, ready-frontier dispatch, fair root budgeting,
      reconciliation, and synthesis from research-only naming into an internal
      host-neutral agentic-workflow core.
...
tests exercise the same core with the answer objective.
...
- [ ] Compile focused read/analysis/critic/synthesizer profiles with minimal
      tools, prompts, context, and response schemas.
...
capability-closure, and context-isolation tests.
...
contradiction, missing detail, truncation, irrelevant candidates, and repair.
...
- [ ] Implement shared queue/steer/stop controls with revision fencing and
      exactly-once control application.
...
- [ ] Propagate one cancellation signal through supervisor, children,
      interpreter, broker, pagination, and scope catalog.
- [ ] Reject late results from cancelled/superseded revisions.
- [ ] Persist ambiguous post-dispatch/pre-commit attempts as `unknown`; never
      automatically repeat them as though no external call occurred.
- [ ] Rehydrate after worker/process restart at every committed phase without
      repeating settled model or Atlassian calls.
- [ ] Preserve existing conversation summarization, prompt caching, and compact
      long-turn context.
...
plus IndexedDB; concurrent steering proves obsolete results cannot reach the
answer.
...
Proof: presenter contract tests plus packed MV3 and CLI event-replay E2E.
...
- [ ] Allocate token/cost/call budgets fairly across dynamic concurrent children
      from one root budget.
...
Proof: deterministic clock tests, slow-provider/tool tests, budget exhaustion,
deadline synthesis, and resumable continuation.
...
- [ ] Build a committed synthetic DOCSY/ATLCLI gold set covering direct,
      decomposition, multi-wave, critique, repair, steering, and deadline cases.
...
Proof: reproducible offline experiment artifact plus operator-owned live review
...
2. **Conversational deadline:** Auto 120 seconds hard; Deep 180 seconds hard,
   both configurable by host/CLI within safe limits. Soft deadlines trigger
   graceful finalization earlier.
