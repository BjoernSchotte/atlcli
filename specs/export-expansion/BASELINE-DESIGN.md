...
**(b) Lösungsansatz.** Kein Feature bauen, sondern eine dreistufige Verifikation: (1) Unit-Invarianten (Feld überlebt, Style-Id stimmt, `updateFields` gesetzt), (2) automatisierter Paket-/Header-Render-Smoke via LibreOffice headless, (3) manuelles Word-Protokoll einmalig pro Release-Train. Gemessene Korrektur (2026-07-22): LibreOffice headless aktualisiert importierte Word-`STYLEREF`-Felder in den verfügbaren Versionen nicht zuverlässig; Stage 2 beweist daher bewusst nur, dass das DOCX konsumierbar und seine Kopfzeile korrekt verbunden ist. Ein proprietärer Word-Runner ist für das OSS-Projekt keine Option. Alternative „eigenes STYLEREF emittieren" bleibt verworfen: das Feld gehört ins Template (Nutzerentscheidung), nicht in die Engine.
...
Zusätzlich ein Golden-Fixture-Template (echtes, in Word gebautes `.docx` mit STYLEREF in `header1.xml`, eingecheckt unter `packages/docx/test-fixtures/styleref-template.docx`). Der LibreOffice-Render-Smoke verwendet eine statische Kopfzeile und prüft nach `soffice --headless --convert-to pdf out.docx` plus `pdftotext`, dass diese auf mindestens zwei Seiten erscheint. Damit werden Paketkonsum und die `headerReference`-Verdrahtung unabhängig geprüft, ohne LibreOffice eine nicht vorhandene STYLEREF-Refresh-Garantie zuzuschreiben. Manuelles Protokoll (Word 365, einmalig): Feld zeigt pro Seite die letzte H1; dokumentieren in `docs/` (Troubleshooting-Sektion: „Kopfzeile zeigt [stale chapter]" → F9/Öffnen-Refresh).
...
**(e) Risiken/offene Fragen.** LibreOffice headless aktualisiert importierte Word-STYLEREF-Felder in den gemessenen Versionen nicht: je nach Version bleibt der Cache stehen oder es erscheint „Reference source not found". Der soffice-Smoke ist deshalb nur Evidenz für Paket-/Header-Rendering; finale Wahrheit der Feldsemantik bleibt das manuelle Word-Protokoll. Offen: Verhalten bei Heading-Promotion (`computeHeadingOffset`, `serialize.ts:243`) — ein Template-STYLEREF auf „Scroll Heading 2" kann nach Promotion leer laufen; Testfall aufnehmen.
