...
**Deviation D7: a rename costs a version.** WP5.1 and WP5.5 assumed a
`PUT /pages/{id}/title` endpoint that changes a title on its own. There is no
such endpoint: on Cloud a title change is an ordinary page update, and on Data
Center likewise. So a rename reads the current storage and writes it back with
the new title, which burns one version exactly as any other edit does. A move
into another directory is separate and does not.

**Deviation D8: the lossy-conversion check is a property of the page.** WP5.11
asks for known losses to be warned about rather than discarded silently. The
check converts the Markdown the reader was served straight back to storage and
compares it against the storage the page actually had, so it detects a real
loss (a two-column `ac:layout` flattened, an inline colour span) and stays quiet
for pages that round-trip — including macros, which do survive. The comparison
ignores whitespace between tags, because the converter re-emits formatting
differently and hashing raw text would flag every page.

- [x] **WP5.1** `src/write-back.ts`: `writeFile` on an existing page parses the frontmatter, converts through `markdownToStorage()`, checks the version against the tree index, and calls `updatePage` with the cached version plus one; on success it updates the tree index and the body cache. A title change in the frontmatter additionally calls `PUT /pages/{id}/title` through a new client method, or carries the title in the same update.
- [x] **WP5.2** Conflict: a version mismatch before the update, or a 409 after it, triggers a refetch and a three-way merge through `merge.ts` with the cached version as base, the written content as ours and the server as theirs. A clean merge retries the update. A merge with conflicts returns `EBUSY` plus a **persisted** conflict file (decision 9) under `<cacheDir>/conflicts/<pageId>-<ts>.md` carrying conflict markers and frontmatter for page ID, base version, server version, creation time and origin, visible in the tree as `<slug>-<id>.conflict.md`. Deleting the conflict file is a local operation, allowed in `ro` mode and without `--allow-delete`. Regression tests: stale write, server 409, clean merge, conflicting merge, a conflict surviving a process restart, and deletion permitted in `ro` mode.
...
- [x] **WP5.10** Audit log: every successful and failed write operation is appended as JSONL carrying timestamp, profile, account ID, operation, path, page ID, source and target version, result and error, written under the cache directory and rotated at 10 MB. Test included.
- [x] **WP5.11** Round-trip test in the fake: read a page, apply a `sed`-style change, write it, read it again, and compare the Markdown after normalization through `normalizeMarkdown`. Known losses, such as macros without a Markdown equivalent, are reported as a warning on stderr rather than discarded silently.
...
an unhandled rejection and killed the process; `readlink` and `realpath`
answered in the core's namespace rather than the mount's, which made every `mv`
fail with "cannot safely determine whether ... is inside ..."; and errors
lacked the `node:fs` code prefix that just-bash's containment check looks for.

**Known upstream rough edge:** `sed -i` catches every write error and reports
"No such file or directory", so an `EROFS` refusal loses its reason through
that one command. A redirect shows the real message. Pinned by a test.
...
- [ ] **WP9.5** *(typecheck, targeted suites and the build are green, and the artifact is measured in `EVIDENCE.md` section 5a; the macOS arm64 and Homebrew checks need those machines)* Run `bun run typecheck`, `bun run test` and `bun run build`, compare bundle size against the previous release, and manually verify the Homebrew formula and the compiled binary on macOS arm64 and Linux x64 with a single listing command.
