 ...
 *
 * **Deliberately NOT re-exported from any barrel.** `src/index.browser.ts` does
 ...
 * being published.
 *
 * Duplicated verbatim in `packages/jira/src/retry-after.ts`: both packages
 * depend on `@atlcli/core` and neither depends on the other, so the honest home
 * is a shared base module. That promotion is queued with the wave-2 shared-base
 * work and this note is the marker for it; duplicating now rather than waiting
 * means neither client is left with the unsafe parse in the meantime.
 */
