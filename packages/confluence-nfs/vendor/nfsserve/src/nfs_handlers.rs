...
pub async fn nfsproc3_readdirplus(
    ...
    // atlcli: reject undersized replies before subtracting protocol overhead.
    ...
    match context.vfs.readdir_with_verifier(dirid, args.cookie, estimated_max_results as usize, verifier).await {
        Ok(result) => {
            ...
            // atlcli: never return an empty non-EOF page that cannot advance.
            if ctr == 0 && (!result.end || !all_entries_written) {
                make_success_reply(xid).serialize(output)?;
                nfs::nfsstat3::NFS3ERR_TOOSMALL.serialize(output)?;
                dir_attr.serialize(output)?;
            } else {
