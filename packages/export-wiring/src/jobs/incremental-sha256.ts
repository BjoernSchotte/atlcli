const UTF8_CHUNK_CODE_UNITS = 64 * 1024;

/** Browser-safe incremental SHA-256 for large checkpoint strings and streams. */
export class IncrementalSha256 {
  ...
  update(input: Uint8Array): void {
    ...
    if (!Number.isSafeInteger(this.#bytes)) throw new RangeError("SHA-256 input is too large.");
    let offset = 0;
  ...
  #compress(block: Uint8Array): void {
    ...
    for (let index = 16; index < 64; index += 1) {
      const x = w[index - 15]!;
      const y = w[index - 2]!;
      const sigma0 = (rotateRight(x, 7) ^ rotateRight(x, 18) ^ (x >>> 3)) >>> 0;
      const sigma1 = (rotateRight(y, 17) ^ rotateRight(y, 19) ^ (y >>> 10)) >>> 0;
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + sum1 + choose + SHA256_CONSTANTS[index]! + w[index]!) >>> 0;
...
export function* utf8Chunks(value: string): Iterable<Uint8Array> {
  const encoder = new TextEncoder();
  for (let offset = 0; offset < value.length;) {
    let end = Math.min(offset + UTF8_CHUNK_CODE_UNITS, value.length);
    if (
      end < value.length &&
      end > offset &&
...
export function sha256Utf8(
  ...
  for (const chunk of utf8Chunks(value)) {
    if (signal?.aborted) {
      throw signal.reason ?? new DOMException("DOCX fingerprinting was cancelled.", "AbortError");
    }
    hasher.update(chunk);
