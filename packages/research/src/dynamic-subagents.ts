...
function compactExcerpt(value, includeCompleteSingleDetail) { const text = typeof value === "string" ? value : ""; if (!text) return { text: "", truncated: false }; if (includeCompleteSingleDetail) return { text: text.slice(0, 32_000), truncated: text.length > 32_000 }; const fragments = [text.slice(0, 640)]; const lower = text.toLowerCase(); for (const term of researchTerms) { const position = lower.indexOf(term.toLowerCase()); if (position < 0) continue; const fragment = text.slice(Math.max(0, position - 180), Math.min(text.length, position + term.length + 260)); if (!fragments.includes(fragment)) fragments.push(fragment); if (fragments.length >= 3) break; } const projected = fragments.join("\\n[… ]\\n"); return { text: projected.length <= 1_200 ? projected : projected.slice(0, 1_200), truncated: projected.length > 1_200 || fragments.length > 1 }; }
...
const includeCompleteSingleDetail = candidates.length === 1 && rawDetails.length === 1;
function projectDetailForModel(detail) { if (!detail || detail.status !== "available" || !detail.value || typeof detail.value !== "object") return { status: "unavailable", sourceId: compactText(detail && detail.sourceId, 200) }; const value = detail.value; const content = value.content && typeof value.content === "object" ? value.content : {}; const links = compactLinks(content.linkTargets); const excerpt = compactExcerpt(content.text, includeCompleteSingleDetail && content.truncated !== true); return { status: "available", source: compactSource(value.source), content: { text: excerpt.text, linkTargets: links.values, linkTargetsTruncated: links.truncated, truncated: content.truncated === true, projectionTruncated: excerpt.truncated } }; }
