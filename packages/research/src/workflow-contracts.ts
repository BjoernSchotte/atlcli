...
> = {
  "focused-researcher": role({
    ...
    description:
      "Acquire bounded Jira or Confluence evidence for one focused branch.",
    phase: "acquisition",
  ...
  "document-distiller": role({
    ...
    description:
      "Distill already accepted source projections without widening scope.",
    phase: "analysis",
    availableFromPhase: "T3",
    allowedCapabilityIds: [],
    supportedOutputSchemas: [
  ...
  "contradiction-verifier": role({
    ...
    description:
      "Verify a bounded contradiction against approved read capabilities.",
    phase: "verification",
    availableFromPhase: "T3",
    allowedCapabilityIds: [
      "jira.issue.search",
  ...
  "coverage-moderator": role({
    ...
    description:
      "Assess bounded coverage and abstention gaps from accepted packets.",
    phase: "verification",
    availableFromPhase: "T3",
    allowedCapabilityIds: [],
    supportedOutputSchemas: [
  ...
  "outline-planner": role({
    id: "outline-planner",
    description:
      "Propose a claim-linked report outline after the V2 evidence store exists.",
    phase: "analysis",
  ...
  reconciler: role({
    id: "reconciler",
    description:
      "Critique accepted packets in a fresh context and return typed defects.",
  ...
  synthesizer: role({
    id: "synthesizer",
    description:
      "Author the one typed report draft from accepted evidence and dispositions.",
    phase: "synthesis",
    availableFromPhase: "T3",
    allowedCapabilityIds: [],
...
function stringArray(value: unknown, label: string, maximum: number): string[] {
  ...
  const result = value.map((item, index) =>
    boundedString(item, `${label}[${index}]`, 240),
  );
...
function assertKeys(
  ...
  if (unexpected)
    invalid(`${label} contains an unexpected field: ${unexpected}.`);
}
...
function parseFindingCandidate(value: unknown): ResearchFindingCandidateV1 {
  ...
  if (item.classification !== "fact" && item.classification !== "inference")
    invalid("Research finding candidate classification is invalid.");
  return {
    id: boundedString(item.id, "Research finding candidate id", 160),
    classification: item.classification,
    summary: boundedString(
      item.summary,
      "Research finding candidate summary",
      800,
    ),
    sourceIds: stringArray(
      item.sourceIds,
      "Research finding candidate sourceIds",
      12,
    ),
  };
}
...
function parseRelationshipCandidate(
  ...
  assertKeys(
    ...
    "Research relationship candidate",
  );
  if (
    item.classification !== "verified" &&
    item.classification !== "hypothesis"
  )
    invalid("Research relationship candidate classification is invalid.");
  return {
    id: boundedString(item.id, "Research relationship candidate id", 160),
    classification: item.classification,
    jiraIssueKey: boundedString(
      item.jiraIssueKey,
      "Research relationship Jira key",
      80,
    ),
    confluenceContentId: boundedString(
      item.confluenceContentId,
      "Research relationship content id",
      120,
    ),
    summary: boundedString(item.summary, "Research relationship summary", 800),
    sourceIds: stringArray(
      item.sourceIds,
      "Research relationship sourceIds",
      12,
    ),
  };
}
...
function parseGap(value: unknown): ResearchGapV1 {
  ...
  return {
    ...
    ...(item.targetId === undefined
      ? {}
      : {
          targetId: boundedString(item.targetId, "Research gap targetId", 160),
        }),
...
function parseFollowUp(value: unknown): ResearchFollowUpProposalV1 {
  ...
  if (!reasonCodes.includes(item.reasonCode as (typeof reasonCodes)[number]))
    invalid("Research follow-up reason code is invalid.");
  return {
    id: boundedString(item.id, "Research follow-up id", 160),
    objective: boundedString(
      item.objective,
      "Research follow-up objective",
      1_000,
    ),
...
export function parseResearchPacketBodyV1(
  value: unknown,
): ResearchPacketBodyV1 {
  ...
  assertKeys(
    ...
    "Research packet body",
  );
  if (packet.schema !== RESEARCH_PACKET_BODY_SCHEMA_V1)
    invalid("Unsupported research packet body schema.");
  if (
    !Array.isArray(packet.findingCandidates) ||
    packet.findingCandidates.length > 24
  )
    invalid("Research finding candidates are invalid.");
  if (
    !Array.isArray(packet.relationshipCandidates) ||
    packet.relationshipCandidates.length > 24
  )
    invalid("Research relationship candidates are invalid.");
  if (!Array.isArray(packet.gaps) || packet.gaps.length > 16)
    invalid("Research gaps are invalid.");
  if (
    !Array.isArray(packet.proposedFollowUps) ||
    packet.proposedFollowUps.length > 3
  )
    invalid("Research follow-up proposals are invalid.");
  const result: ResearchPacketBodyV1 = {
    schema: RESEARCH_PACKET_BODY_SCHEMA_V1,
    answeredQuestion: boundedString(
      packet.answeredQuestion,
    ...
    ...(packet.abstentionReason === undefined
      ? {}
      : {
          abstentionReason: boundedString(
            packet.abstentionReason,
            "Research abstention reason",
            1_000,
          ),
        }),
  };
  ...
  for (const sourceId of referenced)
    if (!result.sourceIds.includes(sourceId))
      invalid("Research packet references an undeclared sourceId.");
  return result;
}
...
function parseResearchQuoteCandidateV2(
  value: unknown,
): ResearchEvidenceQuoteCandidateV2 {
  ...
  return {
    sourceId: boundedString(
      support.sourceId,
      "Research V2 claim support sourceId",
      200,
    ),
...
function parseResearchClaimCandidateV2(
  value: unknown,
): ResearchClaimCandidateV2 {
  ...
  assertKeys(
    ...
    "Research V2 claim candidate",
  );
  if (
    candidate.classification !== "fact" &&
    candidate.classification !== "inference"
  ) {
  ...
  if (new Set(supportKeys).size !== supportKeys.length)
    invalid("Research V2 claim candidate support is duplicated.");
  return {
    id: boundedString(candidate.id, "Research V2 claim candidate id", 160),
    classification: candidate.classification,
    summary: boundedString(
      candidate.summary,
      "Research V2 claim candidate summary",
      2_000,
    ),
    support,
  };
 ...
 */
export function parseResearchPacketModelBodyV2(
  value: unknown,
  ...
  if (
    packet.schema !== RESEARCH_PACKET_BODY_SCHEMA_V2 ||
    !Array.isArray(packet.claimCandidates) ||
    packet.claimCandidates.length > 20 ||
    !Array.isArray(packet.contradictionCandidates) ||
    packet.contradictionCandidates.length > 12 ||
    !Array.isArray(packet.outlineProposals) ||
    packet.outlineProposals.length > 12 ||
    !Array.isArray(packet.gaps) ||
    packet.gaps.length > 16 ||
    !Array.isArray(packet.proposedFollowUps) ||
    packet.proposedFollowUps.length > 3
  ) {
    invalid("Research V2 model packet body is invalid.");
  }
  const claimCandidates = packet.claimCandidates.map(
    parseResearchClaimCandidateV2,
  );
  const candidateIds = new Set(
    claimCandidates.map((candidate) => candidate.id),
  );
  if (candidateIds.size !== claimCandidates.length)
    invalid("Research V2 claim candidate IDs are duplicated.");
  const contradictionIds = new Set<string>();
  const contradictionCandidates = packet.contradictionCandidates.map(
    (value) => {
      const candidate = object(value, "Research V2 contradiction candidate");
      assertKeys(
        candidate,
        ["id", "claimCandidateIds", "summary"],
        "Research V2 contradiction candidate",
      );
      const id = boundedString(
        candidate.id,
        "Research V2 contradiction candidate id",
        160,
      );
      if (contradictionIds.has(id))
        invalid("Research V2 contradiction candidate IDs are duplicated.");
      contradictionIds.add(id);
      const claimCandidateIds = requiredStringArray(
        candidate.claimCandidateIds,
        "Research V2 contradiction claim candidate IDs",
        8,
      );
      if (
        claimCandidateIds.length < 2 ||
        claimCandidateIds.some((claimId) => !candidateIds.has(claimId))
      ) {
        invalid(
          "Research V2 contradiction references an unknown or insufficient claim candidate.",
        );
      }
      return {
        id,
        ...
        summary: boundedString(
          candidate.summary,
          "Research V2 contradiction summary",
          1_200,
        ),
      };
    },
  ...
  const outlineProposals = packet.outlineProposals.map((value) => {
    ...
    const id = boundedString(
      proposal.id,
      "Research V2 outline proposal id",
      160,
    );
    if (outlineIds.has(id))
      invalid("Research V2 outline proposal IDs are duplicated.");
    outlineIds.add(id);
    const claimCandidateIds = stringArray(
      proposal.claimCandidateIds,
      "Research V2 outline claim candidate IDs",
      20,
    );
    ...
    return {
      id,
      sectionId: boundedString(
        proposal.sectionId,
        "Research V2 outline section ID",
        160,
      ),
      title: boundedString(proposal.title, "Research V2 outline title", 240),
      question: boundedString(
        proposal.question,
        "Research V2 outline question",
        1_200,
      ),
      claimCandidateIds,
      dependsOnSectionIds: stringArray(
        proposal.dependsOnSectionIds,
        "Research V2 outline dependencies",
        12,
      ),
      coverageTargetIds: stringArray(
        proposal.coverageTargetIds,
        "Research V2 outline coverage targets",
        32,
      ),
    };
  });
...
export function parseResearchPacketReferenceModelBodyV2(
  ...
  if (
    packet.schema !== RESEARCH_PACKET_REFERENCE_MODEL_SCHEMA_V2 ||
    !Array.isArray(packet.contradictions) ||
    packet.contradictions.length > 12 ||
    !Array.isArray(packet.outlineProposals) ||
    packet.outlineProposals.length > 12 ||
    !Array.isArray(packet.gaps) ||
    packet.gaps.length > 16 ||
  ...
  const claimIds = stringArray(
    packet.claimIds,
    "Research V2 reference claim IDs",
    48,
  ).map((claimId) =>
    claimReferenceId(claimId, "Research V2 reference claim ID"),
  );
  ...
  const contradictions = packet.contradictions.map((value) => {
    ...
    assertKeys(
      candidate,
      ["id", "claimIds", "summary"],
      "Research V2 reference contradiction",
    );
    const id = boundedString(
      candidate.id,
      "Research V2 reference contradiction ID",
      160,
    );
    if (contradictionIds.has(id))
      invalid("Research V2 reference contradiction IDs are duplicated.");
    contradictionIds.add(id);
    const contradictionClaimIds = requiredStringArray(
      ...
      claimReferenceId(claimId, "Research V2 reference contradiction claim ID"),
    );
    if (
      contradictionClaimIds.length < 2 ||
      contradictionClaimIds.some((claimId) => !knownClaims.has(claimId))
    ) {
      invalid(
        "Research V2 reference contradiction references an unknown or insufficient claim.",
      );
    }
    return {
      id,
      ...
      summary: boundedString(
        candidate.summary,
        "Research V2 reference contradiction summary",
        1_200,
      ),
    };
  });
  ...
  const outlineProposals = packet.outlineProposals.map((value) => {
    ...
    const id = boundedString(
      proposal.id,
      "Research V2 reference outline proposal ID",
      160,
    );
    if (proposalIds.has(id))
      invalid("Research V2 reference outline proposal IDs are duplicated.");
    proposalIds.add(id);
    const proposalClaimIds = stringArray(
      ...
      claimReferenceId(claimId, "Research V2 reference outline claim ID"),
    );
    ...
    return {
      ...
      sectionId: boundedString(
        proposal.sectionId,
        "Research V2 reference outline section ID",
        160,
      ),
      title: boundedString(
        proposal.title,
        "Research V2 reference outline title",
        240,
      ),
      question: boundedString(
        proposal.question,
        "Research V2 reference outline question",
        1_200,
      ),
      claimIds: proposalClaimIds,
      dependsOnSectionIds: stringArray(
        proposal.dependsOnSectionIds,
        "Research V2 reference outline dependencies",
        12,
      ),
      coverageTargetIds: stringArray(
        proposal.coverageTargetIds,
        "Research V2 reference outline coverage targets",
        32,
      ),
    };
  });
  return {
    ...
    coverageLimits: stringArray(
      packet.coverageLimits,
      "Research V2 reference coverage limits",
      16,
    ),
    ...(packet.abstentionReason === undefined
      ? {}
      : {
          abstentionReason: boundedString(
            packet.abstentionReason,
            "Research V2 reference abstention reason",
            1_000,
          ),
        }),
  };
...
export function parseResearchPacketBodyV2(
  value: unknown,
): ResearchPacketBodyV2 {
  ...
  const claims = packet.claims.map((value) => {
    ...
    assertKeys(
      claim,
      ["candidateId", "claimId"],
      "Research V2 claim reference",
    );
    return {
      candidateId: boundedString(
        claim.candidateId,
        "Research V2 claim candidate ID",
        160,
      ),
  ...
  if (
    new Set(claims.map((claim) => claim.candidateId)).size !== claims.length ||
  ...
  const referencedClaimIds = stringArray(
    ...
    claimReferenceId(claimId, "Research V2 referenced claim ID"),
  );
  if (
    referencedClaimIds.some((claimId) =>
      claims.some((claim) => claim.claimId === claimId),
    )
  ) {
  ...
  const contradictions = packet.contradictions.map((value) => {
    ...
    assertKeys(
      contradiction,
      ["id", "claimIds", "evidenceIds", "summary"],
      "Research V2 contradiction",
    );
    const id = boundedString(
      contradiction.id,
      "Research V2 contradiction ID",
      160,
    );
    if (contradictionIds.has(id))
      invalid("Research V2 contradiction IDs are duplicated.");
    contradictionIds.add(id);
    const contradictionClaimIds = requiredStringArray(
      contradiction.claimIds,
      "Research V2 contradiction claim IDs",
      8,
    ).map((claimId) =>
      claimReferenceId(claimId, "Research V2 contradiction claim ID"),
    );
    if (
      contradictionClaimIds.length < 2 ||
      contradictionClaimIds.some((claimId) => !claimIds.has(claimId))
    ) {
      invalid(
        "Research V2 contradiction references an unknown or insufficient claim.",
      );
    }
    return {
      id,
      claimIds: contradictionClaimIds,
      evidenceIds: requiredStringArray(
        contradiction.evidenceIds,
        "Research V2 contradiction evidence IDs",
        96,
      ).map((evidenceId) =>
        evidenceReferenceId(
          evidenceId,
          "Research V2 contradiction evidence ID",
        ),
      ),
      summary: boundedString(
  ...
  const outlineProposals = packet.outlineProposals.map((value) => {
    ...
    const proposalClaimIds = stringArray(
      proposal.claimIds,
      "Research V2 outline claim IDs",
      20,
    ).map((claimId) =>
      claimReferenceId(claimId, "Research V2 outline claim ID"),
    );
    if (proposalClaimIds.some((claimId) => !claimIds.has(claimId)))
      invalid("Research V2 outline references an unknown claim.");
    return {
      id,
      sectionId: boundedString(
        proposal.sectionId,
      ...
      evidenceIds: stringArray(
        ...
        "Research V2 outline evidence IDs",
        96,
      ).map((evidenceId) =>
        evidenceReferenceId(evidenceId, "Research V2 outline evidence ID"),
      ),
      dependsOnSectionIds: stringArray(
        proposal.dependsOnSectionIds,
        "Research V2 outline dependencies",
...
export function parseResearchReconciliationInputV1(
  ...
  if (
    input.schema !== RESEARCH_RECONCILIATION_INPUT_SCHEMA_V1 ||
    !Number.isSafeInteger(input.briefRevision) ||
    Number(input.briefRevision) < 1 ||
    !Number.isSafeInteger(input.graphRevision) ||
    Number(input.graphRevision) < 1
  ) {
  ...
  if (projection.kind === "v1-packet-set") {
    ...
    parsedProjection = {
      ...
      gapIds: stringArray(
        projection.gapIds,
        "Research reconciliation gap ids",
        128,
      ),
      sourceIds: stringArray(
        projection.sourceIds,
        "Research reconciliation source ids",
        256,
      ),
    };
    ...
    assertKeys(
      projection,
      ["kind", "claimIds", "evidenceIds", "gapIds"],
      "Research reconciliation projection",
    );
    parsedProjection = {
      kind: "v2-claim-set",
      claimIds: stringArray(
        projection.claimIds,
        "Research reconciliation V2 claim ids",
        128,
      ).map((claimId) =>
        claimReferenceId(claimId, "Research reconciliation V2 claim id"),
      ),
      evidenceIds: stringArray(
        projection.evidenceIds,
        "Research reconciliation V2 evidence ids",
        256,
      ).map((evidenceId) =>
        evidenceReferenceId(
          evidenceId,
          "Research reconciliation V2 evidence id",
        ),
      ),
      gapIds: stringArray(
        projection.gapIds,
        "Research reconciliation V2 gap ids",
        128,
      ),
    };
  } else {
...
export function projectResearchReconciliationInputV1(input: {
  ...
  function appendUniqueCandidateIds(
    ...
    for (const id of ids) {
      if (seen.has(id))
        invalid(
          `Research reconciliation ${label} is duplicated across accepted packets: ${id}.`,
        );
      seen.add(id);
      output.push(id);
    }
  ...
  const packetBodies = input.acceptedPackets.map((packet) => {
    if (
      packet.schema !== RESEARCH_ACCEPTED_PACKET_SCHEMA_V1 ||
      packet.graphRevision !== input.graphRevision
    ) {
      invalid(
        "Research reconciliation input contains a stale or invalid accepted packet.",
      );
    }
    if (seenTaskIds.has(packet.taskId)) {
      invalid(
        `Research reconciliation task is duplicated across accepted packets: ${packet.taskId}.`,
      );
    }
    seenTaskIds.add(packet.taskId);
    if (isResearchPacketBodyV1(packet.body))
      return parseResearchPacketBodyV1(packet.body);
    if (isResearchPacketBodyV2(packet.body))
      return parseResearchPacketBodyV2(packet.body);
    invalid("Research reconciliation input requires research packets.");
  });
  const usesV2 = packetBodies.some(
    (body) => body.schema === RESEARCH_PACKET_BODY_SCHEMA_V2,
  );
  if (usesV2) {
    if (
      packetBodies.some(
      ...
      invalid(
        "Research reconciliation input cannot mix V1 and V2 research packets.",
      );
    }
    ...
    for (const body of packetBodies) {
      ...
      body.contradictions.forEach((contradiction) =>
        contradiction.evidenceIds.forEach((evidenceId) =>
    ...
    return parseResearchReconciliationInputV1({
      ...
      acceptedPacketRefs: input.acceptedPackets.map(
...
function parseReconciliationDefect(
  value: unknown,
  ...
  if (
    !severity.includes(item.severity as (typeof severity)[number]) ||
    !codes.includes(item.code as (typeof codes)[number]) ||
    !actions.includes(item.suggestedAction as (typeof actions)[number]) ||
    !targetKinds.includes(target.kind as (typeof targetKinds)[number])
  )
    invalid("Reconciliation defect enum value is invalid.");
  if (!Array.isArray(item.references) || item.references.length > 16)
    invalid("Reconciliation defect references are invalid.");
  const references = item.references.map(
    (value, index): ResearchSupportRefV1 => {
      const reference = object(value, `Reconciliation reference ${index}`);
      assertKeys(
        reference,
        ["kind", "id"],
        `Reconciliation reference ${index}`,
      );
      if (reference.kind !== "source" && reference.kind !== "evidence")
        invalid("Reconciliation reference kind is invalid.");
      return {
        kind: reference.kind,
        id: boundedString(reference.id, "Reconciliation reference id", 200),
      };
    },
  );
  return {
    ...
    target: {
      kind: target.kind as (typeof targetKinds)[number],
      id: boundedString(target.id, "Reconciliation target id", 160),
    },
    code: item.code as (typeof codes)[number],
    references,
    explanation: boundedString(
      item.explanation,
      "Reconciliation defect explanation",
      1_000,
    ),
...
export function parseReconciliationBodyV1(
  value: unknown,
): ReconciliationBodyV1 {
  ...
  if (body.schema !== RESEARCH_RECONCILIATION_BODY_SCHEMA_V1)
    invalid("Unsupported reconciliation body schema.");
  if (
    !Array.isArray(body.defects) ||
    body.defects.length > 16 ||
    !Array.isArray(body.proposedFollowUps) ||
    body.proposedFollowUps.length > 3
  )
    invalid("Reconciliation body arrays are invalid.");
  return {
...
export function parseResearchReconciliationDispositionV1(
  ...
  if (
    disposition.schema !== RESEARCH_RECONCILIATION_DISPOSITION_SCHEMA_V1 ||
  ...
  return {
    ...
    id: boundedString(
      disposition.id,
      "Research reconciliation disposition id",
      200,
    ),
    ...
    defectId: boundedString(
      disposition.defectId,
      "Research reconciliation defect id",
      160,
    ),
    basedOnGraphRevision: disposition.basedOnGraphRevision as number,
    decision:
      disposition.decision as ResearchReconciliationDispositionV1["decision"],
    reasonCode:
      disposition.reasonCode as ResearchReconciliationDispositionV1["reasonCode"],
    ...(disposition.resultingGraphRevision === undefined
      ? {}
      : {
...
export function parseResearchTaskBodyV1(
  ...
  if (schema === RESEARCH_PACKET_BODY_SCHEMA_V1)
    return parseResearchPacketBodyV1(value);
  if (schema === RESEARCH_PACKET_BODY_SCHEMA_V2)
    return parseResearchPacketBodyV2(value);
  if (schema === RESEARCH_PACKET_REFERENCE_MODEL_SCHEMA_V2)
    return parseResearchPacketBodyV2(value);
  if (schema === RESEARCH_RECONCILIATION_BODY_SCHEMA_V1)
    return parseReconciliationBodyV1(value);
...
export function validateResearchTaskAdmissionV1(input: {
  ...
  if (input.executor === "ptc") {
    ...
    if (input.expectedOutputSchema !== RESEARCH_PACKET_BODY_SCHEMA_V1)
      invalid("A T3 PTC task must return ResearchPacketBodyV1.");
    return;
  }
  ...
  const phase =
    input.phase ??
    (input.expectedOutputSchema === RESEARCH_PACKET_REFERENCE_MODEL_SCHEMA_V2
      ? "T5"
      : "T3");
  if (phase === "T3" && registered.availableFromPhase !== "T3")
    invalid("Research subagent role is unavailable in T3.");
  if (!registered.supportedOutputSchemas.includes(input.expectedOutputSchema))
    invalid("Research subagent output schema is not allowed for its role.");
  for (const capability of input.grantedCapabilityIds) {
    if (!registered.allowedCapabilityIds.includes(capability))
      invalid("Research subagent capability is not allowed for its role.");
  }
}
