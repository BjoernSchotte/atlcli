...
export const RESEARCH_T3_GRAPH_ROLES = RESEARCH_SUBAGENT_ROLE_IDS_V1.filter(
  (roleId) =>
    RESEARCH_SUBAGENT_ROLE_REGISTRY_V1[roleId].availableFromPhase === "T3",
);
...
const PTC_CAPABILITIES = new Set<ResearchGraphCapabilityV1>(
...
function normalizedObjective(objective: string): string {
  if (typeof objective !== "string" || objective.trim() === "")
    invalid("Research brief objective is required.");
  if (objective.length > 4_000)
    invalid("Research brief objective is too long.");
...
function nodeBudget(
  ...
  const roleBudget = roleId
    ...
    : DEFAULT_NODE_BUDGET;
  return {
    maxCapabilityCalls: Math.min(
      roleBudget.maxCapabilityCalls,
      brief.limits.maxPtcCalls,
    ),
    maxInputTokens: Math.min(
      roleBudget.maxInputTokens,
      brief.limits.maxModelInputTokens,
    ),
    maxOutputTokens: Math.min(
      roleBudget.maxOutputTokens,
      brief.limits.maxModelOutputTokens,
    ),
    maxResultBytes: Math.min(
      roleBudget.maxResultBytes,
      brief.limits.maxTotalResponseBytes,
    ),
...
function aggregateBudget(
  nodes: readonly ResearchGraphNodeV1[],
): ResearchNodeBudgetV1 {
  return nodes.reduce<ResearchNodeBudgetV1>(
    (total, node) => ({
      maxCapabilityCalls:
        total.maxCapabilityCalls + node.budget.maxCapabilityCalls,
...
function reconciliationPolicy(
  brief: ResearchBriefV1,
  relation: boolean,
): ResearchGraphReconciliationPolicyV1 {
  ...
  if (brief.resolvedEffort === "deep")
    triggers.push("low_coverage", "contradiction", "stale_or_truncated");
...
function outputSchemaForNode(
  ...
  const hasDetailRead =
    requestedCapabilityIds.includes("jira.issue.get") ||
...
function grantFor(
  ...
  const allowedByRole = seed.roleId
    ...
    : RESEARCH_GRAPH_CAPABILITIES;
  const hostGrant =
    grants?.[seed.id] ?? (seed.roleId ? grants?.[seed.roleId] : undefined);
  return seed.requestedCapabilityIds.filter(
    (capability) =>
      allowedByRole.includes(capability) &&
      (!hostGrant || hostGrant.includes(capability)),
  );
}
...
function nodeFromSeed(
  ...
  return {
    ...
    typedIntentRefs:
      seed.requestedCapabilityIds.length > 0 ? [`intent:${seed.id}`] : [],
    createdFromEvidenceIds: [],
    status:
      graphApproved && seed.dependencies.length === 0
        ? "ready"
        : graphApproved
          ? "blocked"
          : "proposed",
    ...
    completion: {
      requiredCoverageTargetIds: brief.coverageTargets
...
function composeSeeds(
  brief: ResearchBriefV1,
  includeOutlinePlanner: boolean,
): NodeSeed[] {
  ...
  if (brief.resolvedEffort === "lookup") {
    if (jira)
      seeds.push({
        id: "research-node:jira-lookup",
        kind: "search",
        executor: "subagent",
        roleId: "focused-researcher",
        objective:
          "Acquire detail-backed Jira evidence for the exact bounded lookup intent.",
        requestedCapabilityIds: ["jira.issue.search", "jira.issue.get"],
        dependencies: [],
        reasonCodes: ["simple_lookup"],
        priority: 100,
      });
    if (wiki)
      seeds.push({
        id: "research-node:wiki-lookup",
        kind: "search",
        executor: "subagent",
        roleId: "focused-researcher",
        objective:
          "Acquire detail-backed Confluence evidence for the exact bounded lookup intent.",
        requestedCapabilityIds: ["wiki.search", "wiki.page.get"],
        dependencies: [],
        reasonCodes: ["simple_lookup"],
        priority: 100,
      });
    ...
    if (jira)
      seeds.push({
        id: "research-node:jira-research",
        kind: "search",
        executor: "subagent",
        roleId: "focused-researcher",
        objective:
          "Acquire detail-backed Jira evidence for the accepted objective.",
        requestedCapabilityIds: ["jira.issue.search", "jira.issue.get"],
        dependencies: [],
    ...
    if (wiki)
      seeds.push({
        id: "research-node:wiki-research",
        kind: "search",
        executor: "subagent",
        roleId: "focused-researcher",
        objective:
          "Acquire detail-backed Confluence evidence for the accepted objective.",
        requestedCapabilityIds: ["wiki.search", "wiki.page.get"],
        dependencies: [],
        reasonCodes: ["independent_branch"],
        priority: 100,
  ...
  if (relation) {
    seeds.push({
      id: "research-node:cross-product-join",
      kind: "distill",
      executor: "subagent",
      roleId: "document-distiller",
      objective: "Join accepted Jira and Confluence packets without new reads.",
      requestedCapabilityIds: [],
      dependencies: [...researchIds],
      reasonCodes: ["cross_product_join"],
      priority: 80,
    });
  }
  const joinId = seeds.some(
    (seed) => seed.id === "research-node:cross-product-join",
  )
    ? "research-node:cross-product-join"
    : undefined;
  if (contradiction) {
    seeds.push({
      id: "research-node:contradiction-verification",
      kind: "verify",
      executor: "subagent",
      roleId: "contradiction-verifier",
      objective: "Challenge contradiction candidates against accepted packets.",
      requestedCapabilityIds: [],
      dependencies: joinId ? [joinId] : [...researchIds],
      reasonCodes: ["contradiction"],
      priority: 70,
    });
  }
  if (deep) {
    seeds.push({
      id: "research-node:coverage-moderation",
      kind: "moderate",
      executor: "subagent",
      ...
      dependencies: [
        ...seeds
          .filter((seed) => seed.executor !== "ptc" || seed.kind === "search")
          .map((seed) => seed.id),
      ],
  ...
  if (includeOutlinePlanner) {
    seeds.push({
      ...
      roleId: "outline-planner",
      objective:
        "Propose a bounded claim-linked report outline from host-projected current claims only.",
      requestedCapabilityIds: [],
  ...
  const shouldReconcile =
    brief.requestedReconciliation === "required" ||
    (brief.requestedReconciliation === "auto" &&
      (relation || deep || contradiction));
  if (shouldReconcile) {
    seeds.push({
      id: "research-node:reconciler",
      kind: "reconcile",
      executor: "subagent",
      roleId: "reconciler",
      objective:
        "Critique accepted packets in fresh context and return typed defects.",
      requestedCapabilityIds: [],
      dependencies: beforeReconciliation,
      reasonCodes: contradiction ? ["contradiction"] : ["coverage_gap"],
      priority: 40,
    });
    seeds.push({
      ...
      roleId: "contradiction-verifier",
      objective:
        "Execute at most one host-authorized reconciliation follow-up inside the approved Jira and Confluence scope.",
      requestedCapabilityIds: [
  ...
  seeds.push({
    ...
    roleId: "synthesizer",
    objective:
      "Write exactly one typed final report draft from accepted packets and dispositions.",
    requestedCapabilityIds: [],
    dependencies: seeds.map((seed) => seed.id),
    reasonCodes: ["user_requested"],
    priority: 10,
...
export function projectSelectedResearchRolesV1(
  graph: Pick<ResearchGraphV1, "nodes">,
): ResearchSubagentRoleIdV1[] {
  ...
  return graphRolesForNodes(graph.nodes).filter((roleId) =>
    selected.has(roleId),
  );
}

function graphUsesV2Packets(
  nodes: readonly Pick<ResearchGraphNodeV1, "outputSchema">[],
): boolean {
  return nodes.some(
    (node) =>
...
function graphRolesForNodes(
  nodes: readonly Pick<ResearchGraphNodeV1, "outputSchema">[],
): readonly ResearchSubagentRoleIdV1[] {
  return graphUsesV2Packets(nodes)
    ? RESEARCH_T5_GRAPH_ROLES
    : RESEARCH_T3_GRAPH_ROLES;
}

function roleDecisions(
  nodes: ResearchGraphNodeV1[],
): ResearchGraphRoleDecisionV1[] {
...
function approvalEnvelope(
  ...
  const approvedBindings = brief.scopeBindings.filter(
    (binding) =>
  ...
  return {
    ...
    scopeBindingFingerprint: fingerprint(
      approvedBindings.map((binding) => binding.id),
    ),
    ...
    coverageTargetFingerprint: fingerprint(
    ...
    allowedCapabilityIds: [
      ...new Set(nodes.flatMap((node) => node.grantedCapabilityIds)),
    ],
 ...
 */
export function stageResearchGraphForDurableSessionV1(
...
export function composeStandardResearchGraphV1(
  ...
  return composeResearchGraphV1(
    createStandardResearchBriefV1(question, options),
  );
}
...
export function composeResearchGraphV1(
  ...
  if (brief.schema !== RESEARCH_BRIEF_SCHEMA_V1)
    invalid("Unsupported research brief schema.");
  ...
  const relation =
    brief.sourceClasses.includes("jira") &&
    brief.sourceClasses.includes("confluence");
...
export function researchPlanApprovalRequiredV1(
  ...
  if (
    ...
    graph.approvalEnvelope.status !== "proposed"
  ) {
    return undefined;
  }
  return {
    ...
    scopeExpansionMode:
      graph.approvalEnvelope.scopeDiscoveryPolicy.expansionMode,
...
export function assertResearchGraphExecutableV1(graph: ResearchGraphV1): void {
  ...
  if (
    ...
    graph.approvalEnvelope.status !== "approved"
  ) {
    throw new ResearchContractError(
      "invalid-request",
...
function equalBudget(
  left: ResearchNodeBudgetV1,
  right: ResearchNodeBudgetV1,
): boolean {
  return Object.keys(left).every(
    (key) =>
      left[key as keyof ResearchNodeBudgetV1] ===
      right[key as keyof ResearchNodeBudgetV1],
  );
}
...
export function validateResearchGraphV1(graph: ResearchGraphV1): void {
  if (graph.schema !== RESEARCH_GRAPH_SCHEMA_V1)
    invalid("Unsupported research graph schema.");
  if (
    !/^research-session:[A-Za-z0-9._-]{1,120}$/.test(graph.sessionId) ||
    !/^research-turn:[A-Za-z0-9._-]{1,120}$/.test(graph.turnId)
  )
    invalid("Research graph identity is invalid.");
  if (
    !Number.isSafeInteger(graph.revision) ||
    graph.revision < 1 ||
    !Number.isSafeInteger(graph.basedOnBriefRevision) ||
    graph.basedOnBriefRevision < 1
  )
    invalid("Research graph revisions are invalid.");
  if (
    graph.maxParallelNodes < 1 ||
    graph.maxParallelNodes > 3 ||
    graph.maxResearchWaves !== 2 ||
    graph.maxReconciliationWaves !== 1
  )
    invalid("Research graph concurrency or wave limits are invalid.");
  if (graph.nodes.length === 0 || graph.nodes.length > 9)
    invalid("Research graph node count is invalid.");
  if (
    graph.researchWavesCompleted < 0 ||
    graph.researchWavesCompleted > graph.maxResearchWaves ||
    graph.reconciliationWavesCompleted < 0 ||
    graph.reconciliationWavesCompleted > graph.maxReconciliationWaves
  )
    invalid("Research graph completed-wave count is invalid.");
  const ids = new Set<string>();
  for (const node of graph.nodes) {
    if (
      !/^research-node:[A-Za-z0-9._-]{1,120}$/.test(node.id) ||
      ids.has(node.id)
    )
      invalid("Research graph node IDs must be valid and unique.");
    ids.add(node.id);
    if (node.depth !== 0)
      invalid("Research graph depth is unavailable before T6.");
    if (node.kind === "expand")
      invalid(
        "Research graph content-scope expansion is unavailable before T6.",
      );
    if (node.executor === "subagent") {
      if (
        !node.roleId ||
        !graphRolesForNodes(graph.nodes).includes(node.roleId)
      )
        invalid("Research graph subagent role is unknown or unavailable.");
      const role = RESEARCH_SUBAGENT_ROLE_REGISTRY_V1[node.roleId];
      if (
        node.grantedCapabilityIds.some(
          (capability) => !role.allowedCapabilityIds.includes(capability),
        )
      )
        invalid(
          "Research graph grants a capability outside the selected role.",
        );
      if (!role.supportedOutputSchemas.includes(node.outputSchema))
        invalid(
          "Research graph output schema is not allowed for the selected role.",
        );
    } else if (node.roleId) {
    ...
    if (
      node.requestedCapabilityIds.some(
        (capability) => !PTC_CAPABILITIES.has(capability),
      )
    )
      invalid("Research graph requests an unknown capability.");
    if (
      node.grantedCapabilityIds.some(
        (capability) => !node.requestedCapabilityIds.includes(capability),
      )
    )
      invalid("Research graph grants an unrequested capability.");
    if (
      node.requestedCapabilityIds.length > 0 &&
      node.typedIntentRefs.length === 0
    )
      invalid(
        "Research graph capability nodes require typed intent references.",
      );
    if (node.dependencies.includes(node.id))
      invalid("Research graph dependencies must be acyclic.");
    if (
      node.dependencies.some(
        (dependency) =>
          !graph.nodes.some((candidate) => candidate.id === dependency),
      )
    )
      invalid("Research graph dependency is invalid.");
    if (
      node.dependencies.some(
        (dependency) =>
          executionRank(
            graph.nodes.find((candidate) => candidate.id === dependency)!,
          ) >= executionRank(node),
      )
    )
      invalid(
        "Research graph dependency is incompatible with the execution phase.",
      );
    if (
      ...
      node.attempt < 0 ||
      !Number.isSafeInteger(node.maxAttempts) ||
      node.maxAttempts < 1 ||
      node.attempt > node.maxAttempts
    )
      invalid("Research graph node scheduling fields are invalid.");
  ...
  const synthesizers = graph.nodes.filter(
    (node) =>
  ...
  if (synthesizers.length !== 1)
    invalid("Research graph requires exactly one final synthesizer.");
  ...
  for (const decision of graph.roleDecisions) {
    if (
      !availableRoles.includes(decision.roleId) ||
      decisions.has(decision.roleId)
    )
      invalid(
        "Research graph role decisions are missing, duplicated, or unavailable.",
      );
    decisions.set(decision.roleId, decision);
  }
  if (decisions.size !== availableRoles.length)
    invalid(
      "Research graph role decisions are missing, duplicated, or unavailable.",
    );
  ...
  for (const roleId of availableRoles) {
    ...
    if ((decision.decision === "selected") !== projected.has(roleId))
      invalid(
        "Research graph role decisions must derive from executable nodes.",
      );
  }
  if (
    graph.approvalEnvelope.schema !== RESEARCH_APPROVAL_ENVELOPE_SCHEMA_V1 ||
    graph.approvalEnvelope.basedOnGraphRevision !== graph.revision ||
    graph.approvalEnvelope.basedOnBriefRevision !== graph.basedOnBriefRevision
  )
    invalid("Research graph approval envelope revision is invalid.");
  if (
    [...projected].some(
      (roleId) => !graph.approvalEnvelope.allowedRoleIds.includes(roleId),
    )
  )
    invalid("Research graph executable roles exceed the approval envelope.");
  const granted = [
    ...new Set(graph.nodes.flatMap((node) => node.grantedCapabilityIds)),
  ];
  if (
    granted.some(
      (capabilityId) =>
        !graph.approvalEnvelope.allowedCapabilityIds.includes(capabilityId),
    )
  )
    invalid(
      "Research graph executable capabilities exceed the approval envelope.",
    );
  if (
    graph.approvalEnvelope.maxParallelNodes !== graph.maxParallelNodes ||
    graph.approvalEnvelope.maxResearchWaves !== graph.maxResearchWaves ||
    graph.approvalEnvelope.maxReconciliationWaves !==
      graph.maxReconciliationWaves ||
    graph.approvalEnvelope.maxDepth !== 0
  )
    invalid("Research graph approval execution limits are inconsistent.");
  validateBudget(graph.totalBudget, "Research graph total budget");
  const derivedBudget = aggregateBudget(graph.nodes);
  if (!equalBudget(graph.totalBudget, derivedBudget))
    invalid("Research graph total budget must derive from executable nodes.");
  if (
    !budgetWithinCeiling(
      derivedBudget,
      graph.approvalEnvelope.totalBudgetCeiling,
    )
  )
    invalid("Research graph total budget exceeds the approval envelope.");
  if (
    (graph.status === "proposed") !==
    (graph.approvalEnvelope.status === "proposed")
  )
    invalid("Research graph approval status is inconsistent.");
}
...
function parseResearchGraphProposalNodeV1(
  ...
  if (
    !equalSet(Object.keys(record), ["nodeId", "dependencies", "reasonCodes"])
  ) {
    invalid("Research graph proposal node contains unsupported fields.");
  }
  if (
    typeof record.nodeId !== "string" ||
    !/^research-node:[A-Za-z0-9._-]{1,120}$/.test(record.nodeId)
  ) {
    invalid("Research graph proposal node ID is invalid.");
  }
  if (
    !Array.isArray(record.dependencies) ||
    record.dependencies.length > 9 ||
  ...
  if (
    ...
    record.reasonCodes.some(
      (reason) =>
        ...
        !RESEARCH_COMPOSITION_REASONS_V1.includes(
          reason as ResearchCompositionReasonV1,
        ),
    ) ||
...
export function parseResearchGraphProposalV1(
  ...
  if (
    record.schema !== RESEARCH_GRAPH_PROPOSAL_SCHEMA_V1 ||
...
export function acceptResearchGraphProposalV1(
  ...
  if (
    proposal.basedOnBriefRevision !== catalogGraph.basedOnBriefRevision ||
    proposal.basedOnGraphRevision !== catalogGraph.revision
  ) {
    invalid("Research graph proposal revision is stale.");
  }
  if (
    catalogGraph.status !== "approved" ||
    catalogGraph.approvalEnvelope.status !== "approved"
  ) {
    invalid("Research graph proposal requires an approved host envelope.");
  }

  const catalogById = new Map(
    catalogGraph.nodes.map((node) => [node.id, node]),
  );
  const selectedIds = new Set(proposal.nodes.map((node) => node.nodeId));
  if ([...selectedIds].some((nodeId) => !catalogById.has(nodeId))) {
    invalid(
      "Research graph proposal references a node outside the host catalog.",
    );
  }
  if (
    catalogGraph.nodes.some(
      (node) => node.kind === "repair" && selectedIds.has(node.id),
    )
  ) {
    invalid(
      "Conditional reconciliation repair cannot be selected before critique.",
    );
  }
  ...
  if (requiredAcquisitionIds.some((nodeId) => !selectedIds.has(nodeId))) {
    invalid(
      "Research graph proposal must retain every host-required acquisition node.",
    );
  }
  const selectedSynthesizers = catalogGraph.nodes.filter(
    (node) => selectedIds.has(node.id) && node.roleId === "synthesizer",
  );
  if (selectedSynthesizers.length !== 1) {
    invalid("Research graph proposal requires exactly one final synthesizer.");
  }
  const reconciler = catalogGraph.nodes.find(
    (node) => node.roleId === "reconciler",
  );
  if (
    catalogGraph.reconciliationPolicy.mode === "required" &&
    (!reconciler || !selectedIds.has(reconciler.id))
  ) {
    invalid("Research graph proposal must retain required reconciliation.");
  }

  const selectedCatalogNodes = catalogGraph.nodes.filter((node) =>
    selectedIds.has(node.id),
  );
  const proposalById = new Map(
    proposal.nodes.map((node) => [node.nodeId, node]),
  );
  for (const node of selectedCatalogNodes) {
    const proposed = proposalById.get(node.id)!;
    if (
      proposed.dependencies.includes(node.id) ||
    ...
    if (
      mustDependOn.some(
        (dependency) => !proposed.dependencies.includes(dependency),
      )
    ) {
      invalid(
        `Research graph proposal omits a required dependency for ${node.id}.`,
      );
    }
    if (
      (node.kind === "search" || node.kind === "resolve_scope") &&
      proposed.dependencies.length > 0
    ) {
      invalid(
        "Research acquisition nodes cannot depend on later research work.",
      );
    }
  }
...
function readyDependents(nodes: ResearchGraphNodeV1[]): ResearchGraphNodeV1[] {
  const complete = new Set(
    nodes
      .filter((node) => node.status === "complete" || node.status === "pruned")
      .map((node) => node.id),
  );
  return nodes.map((node) =>
    node.status === "blocked" &&
    node.dependencies.every((dependency) => complete.has(dependency))
      ? { ...node, status: "ready" }
      : node,
  );
}
...
export function reduceResearchGraphV1(
  graph: ResearchGraphV1,
  update: ResearchGraphUpdateV1,
): ResearchGraphV1 {
  validateResearchGraphV1(graph);
  if (update.expectedRevision !== graph.revision)
    invalid("Research graph update revision is stale.");
  if (update.kind === "approve") {
    if (
      graph.status !== "proposed" ||
      graph.approvalEnvelope.status !== "proposed"
    )
      invalid("Only a proposed research graph can be approved.");
  ...
  if (update.kind === "activate_repair") {
    if (
      ...
      graph.nodes.some((node) => node.kind === "repair")
    ) {
      invalid(
        "Research repair activation is unavailable for this graph state.",
      );
    }
    const reconciler = graph.nodes.find((node) => node.roleId === "reconciler");
    const synthesizer = graph.nodes.find(
      (node) => node.roleId === "synthesizer",
    );
    if (
      !reconciler ||
      reconciler.status !== "complete" ||
      !synthesizer ||
      (synthesizer.status !== "ready" && synthesizer.status !== "blocked")
    ) {
      invalid(
        "Research repair activation requires a completed reconciler before synthesis.",
      );
    }
    const retained = structuredClone(update.repairNode);
    if (
      retained.kind !== "repair" ||
      retained.executor !== "subagent" ||
      retained.roleId !== "contradiction-verifier" ||
      retained.dependencies.length !== 1 ||
      retained.dependencies[0] !== reconciler.id ||
      retained.status === "complete" ||
      retained.status === "running" ||
      retained.status === "failed" ||
      retained.status === "quarantined"
    ) {
      invalid(
        "Research repair activation does not match the host-retained repair node.",
      );
    }
    const {
  ...
  if (update.kind === "start_node") {
    if (node.status !== "ready")
      invalid("Only a ready research node can start.");
    nodes[index] = { ...node, status: "running", attempt: node.attempt + 1 };
  } else if (update.kind === "complete_node") {
    if (
      node.status !== "running" ||
      !/^packet:[A-Za-z0-9:._-]{1,200}$/.test(update.packetRef)
    )
      invalid("Only a running research node can accept a packet.");
    nodes[index] = { ...node, status: "complete", packetRef: update.packetRef };
  } else if (update.kind === "fail_node") {
    if (node.status !== "running")
      invalid("Only a running research node can fail.");
    nodes[index] = {
      ...node,
      status: "failed",
      stopReason: update.stopReason.slice(0, 200),
    };
  } else if (update.kind === "quarantine_node") {
    if (node.status !== "running")
      invalid("Only a running research node can be quarantined.");
    nodes[index] = {
      ...node,
      status: "quarantined",
      stopReason: update.stopReason.slice(0, 200),
    ...
    if (
      ...
      invalid("Only an undispatched research node can be pruned.");
    nodes[index] = {
      ...node,
      status: "pruned",
      stopReason: update.stopReason.slice(0, 200),
  ...
  const synth = unlocked.find(
    (candidate) => candidate.roleId === "synthesizer",
  );
