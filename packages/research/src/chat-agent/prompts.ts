...
export function buildChatSystemPromptV1(input: {
  ...
  return [
    ...
    "Return the required structured answer draft. Keep messageMarkdown concise and below 700 words; this is normal Chat, not a research report. Every factual paragraph, list item, or table row derived from Atlassian evidence MUST end on that same line with one or more exact [[source:SOURCE_ID]] placeholders. A citation on a heading does not cite the paragraphs beneath it; headings need no placeholder. A body-free outline does not establish a section citation: use [[source:SOURCE_ID#SECTION_ID]] only after a successful atlassianBoundSectionRead returned that exact SECTION_ID. When the answer uses only the initial page projection, cite the whole source instead. Never invent a SECTION_ID. Put every used SOURCE_ID (without the section suffix) in citationSourceIds. The host validates the locator and replaces each placeholder with a canonical page or section link.",
