...
export function buildChatSystemPromptV1(input: {
  ...
  return [
    ...
    input.agenticWorkflowRequired
      ? "The host requires an agentic Chat workflow for this turn. After acknowledging chatStrategyDecide, call tools.chatWorkflowPropose until one proposal is accepted, then never call it again. The proposal may include retrievalPlan with at most three focused variants per bound product: direct question wording, an alternate title or terminology synonym, and a time-window formulation when the question needs it. Mark each variant's expectedInformationGain as high, medium, or low and order it accordingly so bounded acquisition tries the most promising path first. Use only typed text/labels/ancestorId/parentId fields; never emit CQL, JQL, cursors, URLs, tenant data, or broader scope. Bound anchors remain first and never need a search variant. Use only these model-selectable host profile IDs when materially needed: exact-context-reader, confluence-search-reader, jira-search-reader, relationship-tracer, comparison-analyst, contradiction-checker, answer-drafter, answer-critic, chat-synthesizer. The answer-repairer is host-only and must never appear in a proposal. Dependency phases are strict: acquisition readers; parallel analysis; optional contradiction reconciliation; exactly one provisional answer-drafter; exactly one independent answer-critic; then the withheld chat-synthesizer definition. The host augments dependencies, so returned dispatches are authoritative. Parallel profiles in one phase share earlier dependencies and never depend on one another. The proposal must contain exactly one answer-drafter, one answer-critic, and one chat-synthesizer, with no ceremonial task. Strategy, proposal, ready task waves, reviews, and synthesis may use separate eval calls because the QuickJS session and host workflow state persist. Execute each returned ready wave concurrently with awaited Promise.all, and copy description, subagentType, and responseSchema exactly into task(). After acquisition, analysis, and reconciliation complete, call tools.chatStrategyReview({}) before dispatching answer-drafter and answer-critic. After the critic completes, call tools.chatQualityReview({}) exactly once. It returns either the sole final synthesizer dispatch or one host-admitted repair dispatch followed by that synthesizer. Execute them in returned dependency order. Never dispatch the proposal's withheld synthesizer definition directly. Do not answer in the supervisor, create another critic, repair, or synthesizer, or produce a Deep Research report."
