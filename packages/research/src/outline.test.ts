...
async function retainedSupport(
  ...
  const jira = await createResearchEvidenceRecordV1({
    ...
    content: {
      ...
      linkTargets: [
        "https://example.atlassian.net/wiki/spaces/DOCSY/pages/123",
      ],
      truncated: false,
      inputBytes: 72,
    },
  ...
  const createClaim = async (
    record: typeof jira.record,
    chunk: (typeof jira.chunks)[number],
    statement: string,
  ) => {
  ...
  const jiraClaim = await createClaim(
    jira.record,
    jira.chunks[0]!,
    "The Jira issue contains an explicit implementation reference.",
  );
  const wikiClaim = await createClaim(
    wiki.record,
    wiki.chunks[0]!,
    "The Confluence page identifies the implementation ticket.",
  );
...
async function outline(input: {
  ...
  return createResearchOutlineV1({
    ...
    ...(input.supersedesOutlineId === undefined
      ? {}
      : { supersedesOutlineId: input.supersedesOutlineId }),
    ...
    contradictions: [
      {
        ...
        ...(contradictionStatus === "resolved"
          ? {
              resolution:
                "Both details contain the same explicit cross-reference.",
              resolvedAt: "2026-08-01T13:00:04.000Z",
            }
          : {}),
      },
    ],
...
describe("research outline store", () => {
  test("derives coverage and section links only from current retained claims", async () => {
    ...
    expect(persisted.sections[0]!.claimIds).toEqual(
      expect.arrayContaining([support.jiraClaim.id, support.wikiClaim.id]),
    );
  });
  ...
  test("accepts only claim-linked planner structure and revises incomplete coverage host-side", async () => {
      ...
      proposals: [
        {
          ...
          question:
            "What do the retained Jira and Confluence claims establish together?",
          claimIds: [support.jiraClaim.id, support.wikiClaim.id],
    ...
    expect(accepted.outline.sections[0]!.claimIds).toEqual(
      expect.arrayContaining([support.jiraClaim.id, support.wikiClaim.id]),
    );
    expect(accepted.outline.sections[0]!.evidenceIds).toEqual(
      expect.arrayContaining([support.jira.record.id, support.wiki.record.id]),
    );
    const store = new WorkspaceResearchOutlineStoreV1({
      workspace,
      evidenceStore: support.evidence,
    ...
    await expect(store.put(accepted.outline)).resolves.toMatchObject({
    ...
    expect(revised.outline.sections).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "outline-section:host-unassigned",
          claimIds: [support.wikiClaim.id],
        }),
      ]),
    );
  ...
  test("persists a current evidence-linked outline without copying source text into the outline", async () => {
    ...
    const first = new WorkspaceResearchOutlineStoreV1({
      workspace,
      evidenceStore: support.evidence,
      claimLedger: support.claims,
      coverageTargets,
    ...
    const second = new WorkspaceResearchOutlineStoreV1({
      workspace,
      evidenceStore: support.evidence,
      claimLedger: support.claims,
      coverageTargets,
    ...
    const stored = await workspace.readFile(
      `/.atlcli/outlines/v1/outlines/${encodeURIComponent(created.id)}.json`,
    );
  ...
  test("rejects caller-controlled coverage and claims affected by an open contradiction", async () => {
    ...
    const wrongCoverage = structuredClone(
      await outline({
        jiraClaimId: support.jiraClaim.id,
        wikiClaimId: support.wikiClaim.id,
        jiraEvidenceId: support.jira.record.id,
    ...
    await expect(store.put(wrongCoverage)).rejects.toThrow(
      "must be derived from retained evidence",
    );
  });
  ...
  test("does not treat a truncated projection as complete coverage", async () => {
    ...
    const retained = await createResearchEvidenceRecordV1({
      ...
      content: {
        text: "Only a bounded excerpt is retained.",
        linkTargets: [],
        truncated: true,
        inputBytes: 100_000,
      },
    ...
    await expect(store.put(partial)).resolves.toMatchObject({
      coverage: [{ status: "partial" }],
    });
    const overstated = structuredClone(partial);
    overstated.coverage[0]!.status = "covered";
    await expect(store.put(overstated)).rejects.toThrow(
      "must be derived from retained evidence",
    );
  });
  ...
  test("blocks publication when an already-persisted outline loses supporting evidence", async () => {
    ...
    await expect(store.validateCurrent()).rejects.toThrow(
      "claim that is not current",
    );
    await expect(
      support.claims.get(support.jiraClaim.id),
  ...
  test("retains the previously published outline after an interrupted pointer update", async () => {
    ...
    const writer = new WorkspaceResearchOutlineStoreV1({
    ...
    await expect(writer.put(second)).rejects.toThrow(
      "injected outline index interruption",
    );
    const recovered = new WorkspaceResearchOutlineStoreV1({
      workspace: durableWorkspace,
      evidenceStore: support.evidence,
      claimLedger: support.claims,
      coverageTargets,
    });
    await expect(recovered.current()).resolves.toMatchObject({
      id: first.id,
      revision: 1,
    });
