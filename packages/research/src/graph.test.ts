...
import { RESEARCH_ONE_SHOT_POLICY_SCHEMA_V1 } from "./contracts.js";
import {
...
describe("dynamic research graph composition", () => {
  test("rejects an unresolved empty scope before graph composition", () => {
    expect(() =>
      composeStandardResearchGraphV1("Research the relevant work.", {
        scope: {
          siteOrigin: "https://example.atlassian.net",
          jiraProjectKeys: [],
          confluenceSpaceKeys: [],
  ...
  test("gives every productive host the same standard cross-product graph", () => {
    ...
    expect(
      graph.nodes.filter((node) => node.roleId === "focused-researcher"),
    ).toHaveLength(2);
  });
  ...
  test("binds productive graph budgets and scope to the normalized host request", () => {
    ...
    expect(
      graph.nodes.find((node) => node.roleId === "focused-researcher")?.budget
        .maxDurationMs,
    ).toBe(180_000);
  ...
  test("selects structurally different nodes for lookup, Jira-only, and cross-product briefs", () => {
    const lookup = composeResearchGraphV1(
      brief("Get Jira issue DEMO-1", ["jira"], "off", "lookup"),
    );
    const jiraOnly = composeResearchGraphV1(
      brief("List open Jira tickets", ["jira"], "off"),
    );
    const crossProduct = composeResearchGraphV1(
      brief("Which Confluence content is related to Jira tickets?", [
        "jira",
        "confluence",
      ]),
    );
    expect(projectSelectedResearchRolesV1(lookup)).toEqual([
      "focused-researcher",
      "synthesizer",
    ]);
    expect(
      lookup.nodes.filter((node) => node.roleId === "focused-researcher"),
    ).toHaveLength(1);
    expect(
      lookup.nodes.find((node) => node.id === "research-node:jira-lookup")
        ?.grantedCapabilityIds,
    ).toEqual(["jira.issue.search", "jira.issue.get"]);
    expect(projectSelectedResearchRolesV1(jiraOnly)).toEqual([
      "focused-researcher",
      "synthesizer",
    ]);
    ...
    expect(
      crossProduct.nodes.find((node) => node.roleId === "synthesizer")
        ?.dependencies,
    ).toEqual([
  ...
  test("uses quote-bearing V2 for detail reads and references-only V2 for analysis", () => {
    const graph = composeResearchGraphV1(
      brief("Join bounded Jira and Confluence evidence.", [
        "jira",
        "confluence",
      ]),
    ...
    expect(
      detailNodes.every(
        (node) => node.outputSchema === RESEARCH_PACKET_BODY_SCHEMA_V2,
      ),
    ).toBe(true);
    expect(
      graph.nodes.find((node) => node.roleId === "document-distiller")
        ?.outputSchema,
    ).toBe(RESEARCH_PACKET_REFERENCE_MODEL_SCHEMA_V2);
    expect(
      graph.nodes.find((node) => node.roleId === "reconciler")?.outputSchema,
    ).toBe("atlcli.reconciliation-body/v1");
    expect(
      graph.nodes.find((node) => node.roleId === "synthesizer")?.outputSchema,
  ...
  test("offers an optional T5 outline planner only to V2 graphs and fences it before critique", () => {
    ...
    const accepted = acceptResearchGraphProposalV1(
      graph,
      proposalFor(graph, selected),
    );
    const acceptedPlanner = accepted.nodes.find(
      (node) => node.id === planner.id,
    )!;
    ...
    expect(
      accepted.nodes.find((node) => node.roleId === "reconciler")?.dependencies,
  ...
  test("persists the host-granted intersection rather than widening model requests", () => {
    ...
    expect(graph.approvalEnvelope.allowedCapabilityIds).toEqual([
      "jira.issue.search",
    ]);
  });

  test("selects dedicated joining, verification, and moderation for task-shaped briefs", () => {
    const standard = composeResearchGraphV1(
      brief(
        "How do these pages describe the funnel, and which DEMO work items correspond to each stage?",
        ["jira", "confluence"],
      ),
    );
    expect(projectSelectedResearchRolesV1(standard)).toContain(
      "document-distiller",
    );
    const deep = composeResearchGraphV1(
      brief(
        "How do these pages describe the funnel, and which DEMO work items correspond to each stage?",
        ["jira", "confluence"],
        "auto",
    ...
    const contradiction = composeResearchGraphV1(
      brief("Which Confluence content explicitly contradicts Jira tickets?", [
        "jira",
        "confluence",
      ]),
    );
    expect(projectSelectedResearchRolesV1(contradiction)).toContain(
      "contradiction-verifier",
    );

    const explicitRelationship = composeResearchGraphV1(
      brief("Which Confluence pages are explicitly related to Jira tickets?", [
        "jira",
        "confluence",
      ]),
    );
  ...
  test("derives every visible role decision from executable nodes", () => {
    ...
    expect(
      graph.roleDecisions.find((entry) => entry.roleId === "focused-researcher")
        ?.decision,
    ).toBe("selected");
    expect(
      graph.roleDecisions.find((entry) => entry.roleId === "document-distiller")
        ?.decision,
    ).toBe("omitted");
    expect(() =>
      validateResearchGraphV1({
        ...graph,
        roleDecisions: graph.roleDecisions.filter(
          (entry) => entry.roleId !== "coverage-moderator",
        ),
      }),
    ).toThrow("missing");
    expect(() =>
      validateResearchGraphV1({
        ...graph,
        roleDecisions: graph.roleDecisions.map((entry) =>
          entry.roleId === "focused-researcher"
            ? { ...entry, decision: "omitted" as const }
            : entry,
        ),
      }),
    ).toThrow("derive");
  ...
  test("accepts structurally different supervisor compositions inside one host envelope", () => {
    ...
    const concise = acceptResearchGraphProposalV1(
      catalog,
      proposalFor(catalog, requiredIds),
    );
    const critical = acceptResearchGraphProposalV1(
      catalog,
      proposalFor(catalog, [
    ...
    expect(
      concise.nodes.find((node) => node.roleId === "synthesizer")?.dependencies,
    ).toEqual(requiredIds.slice(0, 2));
    expect(
      critical.nodes.find((node) => node.roleId === "reconciler")?.dependencies,
    ).toEqual([
      "research-node:jira-research",
      "research-node:wiki-research",
      "research-node:cross-product-join",
  ...
  test("rejects supervisor graph widening and invalid critical topology", () => {
    const catalog = composeResearchGraphV1(
      brief("Which Jira and Confluence items correspond?", [
        "jira",
        "confluence",
      ]),
    );
    ...
    expect(() =>
      acceptResearchGraphProposalV1(catalog, {
        ...valid,
        nodes: valid.nodes.map((node) =>
          node.nodeId === "research-node:jira-research"
    ...
    expect(() =>
      acceptResearchGraphProposalV1(catalog, {
        ...valid,
        nodes: valid.nodes.filter(
          (node) => node.nodeId !== "research-node:wiki-research",
        ),
      }),
    ...
    expect(() =>
      acceptResearchGraphProposalV1(catalog, {
        ...valid,
        nodes: valid.nodes.map((node) =>
          node.nodeId === "research-node:cross-product-join"
  ...
  test("rejects cycles, unknown dependencies, depth, and capability widening", () => {
    ...
    const cyclic = {
      ...graph,
      nodes: graph.nodes.map((node) => ({ ...node, dependencies: [node.id] })),
    };
    ...
    const depth = {
      ...
      nodes: graph.nodes.map((node, index) => ({
        ...node,
        depth: index === 0 ? (1 as const) : node.depth,
      })),
    };
    ...
    expect(() =>
      validateResearchGraphV1({
        ...
        totalBudget: {
          ...graph.totalBudget,
          maxCapabilityCalls: graph.totalBudget.maxCapabilityCalls + 1,
        },
      }),
    ).toThrow("derive");
  });
  ...
  test("keeps required plan approval proposed until the exact revision is approved", () => {
    const proposed = composeResearchGraphV1(
      brief(
    ...
    expect(proposed).toMatchObject({
      status: "proposed",
      approvalEnvelope: { status: "proposed" },
    });
    expect(proposed.nodes.every((node) => node.status === "proposed")).toBe(
      true,
    );
    expect(() =>
      reduceResearchGraphV1(proposed, {
        kind: "approve",
        expectedRevision: 2,
        approvedAt: "2026-01-02T00:00:00.000Z",
      }),
    ).toThrow("stale");
    const approved = reduceResearchGraphV1(proposed, {
      kind: "approve",
      expectedRevision: 1,
      approvedAt: "2026-01-02T00:00:00.000Z",
    });
  ...
  test("uses a pure revision-fenced reducer to unlock dependency barriers", () => {
    let graph = composeResearchGraphV1(
      brief("List Jira tickets", ["jira"], "off"),
    );
    graph = reduceResearchGraphV1(graph, {
      kind: "start_node",
      expectedRevision: 1,
      nodeId: "research-node:jira-research",
    });
    expect(
      graph.nodes.find((node) => node.id === "research-node:synthesizer")
        ?.status,
    ).toBe("blocked");
    graph = reduceResearchGraphV1(graph, {
      kind: "complete_node",
    ...
    graph = reduceResearchGraphV1(graph, {
      kind: "start_node",
      expectedRevision: 1,
      nodeId: "research-node:synthesizer",
    });
    graph = reduceResearchGraphV1(graph, {
      kind: "complete_node",
      expectedRevision: 1,
      nodeId: "research-node:synthesizer",
      packetRef: "packet:task:2",
    });
    expect(graph.status).toBe("complete");
  });

  test("keeps graph failure transitions revision-fenced and immutable", () => {
    const initial = composeResearchGraphV1(
      brief("List Jira tickets", ["jira"], "off"),
    );
    const snapshot = structuredClone(initial);
    expect(() =>
      reduceResearchGraphV1(initial, {
        kind: "start_node",
        expectedRevision: 2,
        nodeId: "research-node:jira-research",
      }),
    ...
    expect(
      quarantined.nodes.find(
        (node) => node.id === "research-node:jira-research",
      ),
    ).toMatchObject({ status: "quarantined", stopReason: "late-result" });
    expect(
      quarantined.nodes.find((node) => node.id === "research-node:synthesizer")
        ?.status,
    ).toBe("blocked");
    expect(() =>
      reduceResearchGraphV1(quarantined, {
        kind: "complete_node",
        expectedRevision: 1,
        nodeId: "research-node:jira-research",
