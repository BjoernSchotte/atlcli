...
export const RESEARCH_CONTRADICTION_SCHEMA_V1 =
  "atlcli.research-contradiction/v1" as const;
export const RESEARCH_COVERAGE_ASSESSMENT_SCHEMA_V1 =
  "atlcli.research-coverage-assessment/v1" as const;
export const RESEARCH_OUTLINE_SCHEMA_V1 = "atlcli.research-outline/v1" as const;
export const RESEARCH_OUTLINE_STORE_INDEX_SCHEMA_V1 =
  "atlcli.research-outline-store-index/v1" as const;
...
export interface ResearchOutlineStoreV1 {
  ...
  list(input?: {
    limit?: number;
    cursor?: string;
  }): Promise<{ outlines: ResearchOutlineV1[]; nextCursor?: string }>;
  clear(): Promise<void>;
}
...
function sectionId(value: unknown): string {
  ...
  if (!/^outline-section:[A-Za-z0-9._-]{1,128}$/.test(result))
    invalid("Outline section ID is invalid.");
  return result;
}

function contradictionId(value: unknown): string {
  const result = text(value, "Contradiction ID", 160);
  if (!/^contradiction:[A-Za-z0-9._-]{1,128}$/.test(result))
    invalid("Contradiction ID is invalid.");
  return result;
}

function claimId(value: unknown): string {
  const result = text(value, "Outline claim ID", 96);
  if (!/^claim:[a-f0-9]{48}$/.test(result))
    invalid("Outline claim ID is invalid.");
  return result;
}

function evidenceId(value: unknown): string {
  const result = text(value, "Outline evidence ID", 96);
  if (!/^evidence:[a-f0-9]{48}$/.test(result))
    invalid("Outline evidence ID is invalid.");
  return result;
}

function coverageTargetId(value: unknown): string {
  const result = text(value, "Coverage target ID", 160);
  if (!/^coverage:[A-Za-z0-9._-]{1,120}$/.test(result))
    invalid("Coverage target ID is invalid.");
  return result;
}
...
function parseContradiction(value: unknown): ResearchContradictionV1 {
  ...
  if (candidate.schema !== RESEARCH_CONTRADICTION_SCHEMA_V1)
    invalid("Contradiction schema is invalid.");
  if (
  ...
  if (claimIds.length < 2)
    invalid("A contradiction requires at least two claims.");
  const evidenceIds = sortedDistinct(
    candidate.evidenceIds,
    "Contradiction evidence IDs",
    MAXIMUM_EVIDENCE_PER_REFERENCE,
    evidenceId,
    (id) => id,
  );
  ...
  if (status === "open") {
    if (
      ...
      invalid("An open contradiction cannot have a resolution.");
  } else if (
    candidate.resolution === undefined ||
    candidate.resolvedAt === undefined
  ) {
...
function parseCoverage(value: unknown): ResearchCoverageAssessmentV1 {
  if (!value || typeof value !== "object")
    invalid("Coverage assessment is invalid.");
  const candidate = value as Partial<ResearchCoverageAssessmentV1>;
  if (candidate.schema !== RESEARCH_COVERAGE_ASSESSMENT_SCHEMA_V1)
    invalid("Coverage assessment schema is invalid.");
  if (
    candidate.status !== "covered" &&
    candidate.status !== "partial" &&
    candidate.status !== "uncovered"
  ) {
  ...
  return {
    ...
    distinctSourceCount: nonNegativeInteger(
...
function parseSection(value: unknown): ResearchOutlineSectionV1 {
  if (!value || typeof value !== "object")
    invalid("Outline section is invalid.");
  const candidate = value as Partial<ResearchOutlineSectionV1>;
  const claimIds = sortedDistinct(
    candidate.claimIds,
    "Outline section claim IDs",
    MAXIMUM_CLAIMS_PER_REFERENCE,
    claimId,
    (id) => id,
  );
  const evidenceIds = sortedDistinct(
    candidate.evidenceIds,
    "Outline section evidence IDs",
    MAXIMUM_EVIDENCE_PER_REFERENCE,
    evidenceId,
    (id) => id,
  );
  const contradictionIds = sortedDistinct(
    candidate.contradictionIds,
    "Outline section contradiction IDs",
    MAXIMUM_CONTRADICTIONS,
    contradictionId,
    (id) => id,
  );
  const coverageTargetIds = sortedDistinct(
    candidate.coverageTargetIds,
    "Outline section coverage target IDs",
  ...
  if (
    ...
    invalid(
      "Outline section has no evidence, claim, contradiction, or coverage link.",
    );
  }
  return {
    ...
    dependsOnSectionIds: sortedDistinct(
      candidate.dependsOnSectionIds,
      "Outline section dependencies",
      MAXIMUM_SECTIONS,
      sectionId,
      (id) => id,
    ),
...
function parseOutline(value: unknown): ResearchOutlineV1 {
  ...
  if (candidate.schema !== RESEARCH_OUTLINE_SCHEMA_V1)
    invalid("Outline schema is invalid.");
  const sections = sortedDistinct(
    candidate.sections,
    "Outline sections",
    MAXIMUM_SECTIONS,
    parseSection,
  ...
  const contradictions = sortedDistinct(
  ...
  for (const section of sections) {
    if (
      section.dependsOnSectionIds.includes(section.id) ||
      section.dependsOnSectionIds.some((id) => !sectionIds.has(id))
    ) {
  ...
  const visit = (id: string): void => {
    if (visiting.has(id))
      invalid("Outline section dependencies must be acyclic.");
  ...
  return {
    ...
    basedOnBriefRevision: positiveInteger(
      candidate.basedOnBriefRevision,
      "Outline brief revision",
      1_000_000,
    ),
    ...(candidate.supersedesOutlineId === undefined
      ? {}
      : { supersedesOutlineId: outlineId(candidate.supersedesOutlineId) }),
...
function parseIndex(value: string): PersistedOutlineIndexV1 {
  ...
  if (!parsed || typeof parsed !== "object")
    invalid("Outline index is invalid.");
  const candidate = parsed as Partial<PersistedOutlineIndexV1>;
  if (candidate.schema !== RESEARCH_OUTLINE_STORE_INDEX_SCHEMA_V1)
    invalid("Outline index is invalid.");
  const outlineIds = sortedDistinct(
    candidate.outlineIds,
    "Outline index IDs",
    MAXIMUM_OUTLINES,
    outlineId,
    (id) => id,
  ...
  const currentOutlineId =
    candidate.currentOutlineId === undefined
      ? undefined
      : outlineId(candidate.currentOutlineId);
  if (currentOutlineId !== undefined && !outlineIds.includes(currentOutlineId))
    invalid("Outline index current ID is invalid.");
  return {
    schema: RESEARCH_OUTLINE_STORE_INDEX_SCHEMA_V1,
    outlineIds,
    ...(currentOutlineId === undefined ? {} : { currentOutlineId }),
  };
}
 ...
 */
export async function createResearchOutlineV1(
  input: Omit<ResearchOutlineV1, "schema" | "id">,
  ...
  const canonical = {
    ...
    ...(normalized.supersedesOutlineId === undefined
      ? {}
      : { supersedesOutlineId: normalized.supersedesOutlineId }),
...
function coverageTargetMap(
  targets: readonly ResearchCoverageTargetV1[],
): Map<string, ResearchCoverageTargetV1> {
  if (
    !Array.isArray(targets) ||
    targets.length === 0 ||
    targets.length > MAXIMUM_COVERAGE_TARGETS
  ) {
  ...
  const entries = targets.map((target) => {
    ...
    const question = text(
      candidate.question,
      "Outline coverage target question",
      1_000,
    );
    ...
    if (
      ...
      minimumDistinctSources > MAXIMUM_EVIDENCE_PER_REFERENCE ||
      !Array.isArray(sourceClasses) ||
  ...
  if (new Set(entries.map(([id]) => id)).size !== entries.length)
    invalid("Outline coverage targets contain duplicates.");
  return new Map(entries);
}

function expectedCoverageStatus(
  target: ResearchCoverageTargetV1,
  records: readonly ResearchEvidenceRecordV1[],
): ResearchCoverageStatusV1 {
  const completeRecords = records.filter((record) => !record.version.truncated);
  const sources = new Set(
    completeRecords.map((record) => record.identity.canonicalId),
  );
  const products = new Set(
    completeRecords.map((record) => record.identity.product as ResearchProduct),
  );
  if (
    sources.size >= target.minimumDistinctSources &&
    target.sourceClasses.every((product) => products.has(product))
  )
    return "covered";
...
export async function createResearchOutlineFromClaimsV1(input: {
  ...
  for (const id of uniqueClaimIds) {
    ...
    const additions = claim.evidenceIds.filter(
      (evidenceId) => !selectedEvidenceIds.has(evidenceId),
    );
    if (
      selectedEvidenceIds.size + additions.length >
      MAXIMUM_EVIDENCE_PER_REFERENCE
    )
      continue;
  ...
  const coverage = [...targets.values()].map(
    (target): ResearchCoverageAssessmentV1 => {
      const matchingEvidenceIds = evidenceIds.filter((evidenceId) =>
        target.sourceClasses.includes(
          records.get(evidenceId)!.identity.product,
        ),
      );
      const matchingEvidence = matchingEvidenceIds.map(
        (evidenceId) => records.get(evidenceId)!,
      );
      const matchingClaimIds = selectedClaims
        .filter((claim) =>
          claim.evidenceIds.some((evidenceId) =>
            matchingEvidenceIds.includes(evidenceId),
          ),
        )
        .map((claim) => claim.id);
      return {
        ...
        distinctSourceCount: new Set(
          matchingEvidence.map((record) => record.identity.canonicalId),
        ).size,
...
export async function resolveResearchOutlineProposalV1(input: {
  ...
  if (!Number.isFinite(Date.parse(input.checkedAt)))
    invalid("Outline proposal check time is invalid.");
  if (input.proposals.length === 0) {
    return {
      disposition: "rejected",
      reason: "no-proposals",
      outline: clone(baseline),
    };
  }
  ...
  try {
    ...
    const permittedCoverageTargetIds = new Set(
      baseline.coverage.map((coverage) => coverage.targetId),
    );
    const modelSectionIds = new Set<string>();
    const proposedClaimIds = new Set<string>();
    const normalized = input.proposals.map((proposal) => {
      const modelSectionId = text(
        proposal.sectionId,
        "Outline proposal section ID",
        160,
      );
      if (modelSectionIds.has(modelSectionId))
        invalid("Outline proposal section IDs are duplicated.");
      ...
      if (
        claimIds.length === 0 ||
        claimIds.some((id) => !permittedClaimIds.has(id))
      ) {
    ...
    if (
      normalized.some(
        (proposal) =>
          proposal.dependsOnSectionIds.includes(proposal.modelSectionId) ||
    ...
    for (const id of permittedClaimIds) {
      ...
      if (!claim || claim.freshness !== "current") {
        throw new ResearchContractError(
          "invalid-report",
          "Outline proposal references a claim that is no longer current.",
        );
      }
      claims.set(id, claim);
    }
    const hostSectionIdByModelId = new Map(
      normalized.map((proposal, index) => [
        proposal.modelSectionId,
        `outline-section:planned-${index + 1}`,
      ]),
    );
    ...
    const sections: ResearchOutlineSectionV1[] = normalized.map((proposal) => ({
      ...
      dependsOnSectionIds: proposal.dependsOnSectionIds.map(
        (id) => hostSectionIdByModelId.get(id)!,
      ),
    }));
    const unassignedClaimIds = [...permittedClaimIds]
      .filter((id) => !proposedClaimIds.has(id))
      .sort();
    const assignedCoverageTargetIds = new Set(
      sections.flatMap((section) => section.coverageTargetIds),
    );
    const unassignedCoverageTargetIds = [...permittedCoverageTargetIds]
      .filter((id) => !assignedCoverageTargetIds.has(id))
      .sort();
    const revised =
      unassignedClaimIds.length > 0 || unassignedCoverageTargetIds.length > 0;
    if (revised) {
      sections.push({
        id: "outline-section:host-unassigned",
        title: "Additional validated findings",
        question:
          "Which validated claims or coverage targets were not assigned by the outline proposal?",
    ...
    return {
      ...
      outline: await createResearchOutlineV1({
        ...
        ...(baseline.supersedesOutlineId === undefined
          ? {}
          : { supersedesOutlineId: baseline.supersedesOutlineId }),
...
export class WorkspaceResearchOutlineStoreV1 implements ResearchOutlineStoreV1 {
  ...
  async #persistIndex(): Promise<void> {
    ...
    if (bytes(contents) > MAXIMUM_INDEX_BYTES) {
      throw new ResearchContractError(
        "limit-exceeded",
        "Outline store index is too large.",
      );
    }
  ...
  async #claims(ids: readonly string[]): Promise<Map<string, ResearchClaimV1>> {
    ...
    const entries = await Promise.all(
      ids.map(
        async (id) =>
          [id, await this.#claimLedger.refresh(id, checkedAt)] as const,
      ),
    );
    const claims = new Map<string, ResearchClaimV1>();
    for (const [id, claim] of entries) {
      ...
      if (claim.freshness !== "current") {
        throw new ResearchContractError(
          "invalid-report",
          "Outline references a claim that is not current.",
        );
      }
      claims.set(id, claim);
    }
  ...
  async #evidence(
    ids: readonly string[],
  ): Promise<Map<string, ResearchEvidenceRecordV1>> {
    const entries = await Promise.all(
      ids.map(async (id) => [id, await this.#evidenceStore.get(id)] as const),
    );
    const records = new Map<string, ResearchEvidenceRecordV1>();
  ...
  async #validateReferences(
    ...
    const coverageIds = new Set(
      validated.coverage.map((entry) => entry.targetId),
    );
    const expectedCoverageIds = new Set(this.#coverageTargets.keys());
    if (
      coverageIds.size !== expectedCoverageIds.size ||
      [...expectedCoverageIds].some((id) => !coverageIds.has(id))
    ) {
    ...
    for (const claim of claims.values())
      claim.evidenceIds.forEach((id) => allEvidenceIds.add(id));
    const records = await this.#evidence([...allEvidenceIds]);
    if (
      new Set(
        [...records.values()].map((record) => record.identity.tenantOrigin),
      ).size > 1
    ) {
      invalid("Outline evidence cannot mix tenants.");
    }
    const contradictions = new Map(
      validated.contradictions.map((entry) => [entry.id, entry]),
    );
    for (const contradiction of validated.contradictions) {
      const permittedEvidence = new Set(
        contradiction.claimIds.flatMap((id) => claims.get(id)!.evidenceIds),
      );
    ...
    for (const coverage of validated.coverage) {
      ...
      const coverageRecords = coverage.evidenceIds.map(
        (id) => records.get(id)!,
      );
      const permittedEvidence = new Set(
        coverage.claimIds.flatMap((id) => claims.get(id)!.evidenceIds),
      );
      if (
        coverage.claimIds.length > 0 &&
        coverage.evidenceIds.some((id) => !permittedEvidence.has(id))
      ) {
      ...
      const expectedSources = new Set(
        coverageRecords.map((record) => record.identity.canonicalId),
      ).size;
      if (
        coverage.status !== expectedStatus ||
        coverage.distinctSourceCount !== expectedSources
      ) {
        invalid("Coverage assessment must be derived from retained evidence.");
      }
    }
    for (const section of validated.sections) {
      if (section.contradictionIds.some((id) => !contradictions.has(id)))
        invalid("Outline section references an unknown contradiction.");
      if (section.coverageTargetIds.some((id) => !expectedCoverageIds.has(id)))
        invalid("Outline section references an unknown coverage target.");
      const permittedEvidence = new Set(
        section.claimIds.flatMap((id) => claims.get(id)!.evidenceIds),
      );
      ...
      if (section.claimIds.some((id) => openClaimIds.has(id))) {
        invalid(
          "Outline section cannot publish a claim with an open contradiction.",
        );
      }
      const unknownOpenContradiction = validated.contradictions.find(
        (contradiction) =>
          contradiction.status === "open" &&
          contradiction.claimIds.some((id) => section.claimIds.includes(id)) &&
          !section.contradictionIds.includes(contradiction.id),
      );
      if (unknownOpenContradiction) {
        invalid(
          "Outline section cannot use a claim affected by an open contradiction.",
        );
      }
    }
    return validated;
    ...
    return this.#exclusive(async () => {
      ...
      if (existing) {
        if (JSON.stringify(existing) !== JSON.stringify(validated))
          invalid("Outline ID collides with a different revision.");
        return clone(existing);
      }
      if (this.#index.outlineIds.length >= MAXIMUM_OUTLINES) {
        throw new ResearchContractError(
          "limit-exceeded",
          "Outline store limit is exhausted.",
        );
      }
      const previous =
        this.#index.currentOutlineId === undefined
          ? undefined
          : await this.#readOutline(this.#index.currentOutlineId);
      if (this.#index.currentOutlineId !== undefined && !previous)
        invalid("Outline index references a missing outline.");
      if (!previous) {
        if (
          validated.revision !== 1 ||
          validated.supersedesOutlineId !== undefined
        ) {
          invalid(
            "The first outline must be revision one without a predecessor.",
          );
        }
      } else if (
        validated.revision !== previous.revision + 1 ||
        validated.supersedesOutlineId !== previous.id
      ) {
        invalid(
          "Outline revisions must directly supersede the current revision.",
        );
      }
      try {
        await this.#workspace.writeFile(
          outlinePath(validated.id),
  ...
  async list(
    ...
    return this.#exclusive(async () => {
      ...
      const cursor =
        input.cursor === undefined ? undefined : outlineId(input.cursor);
      const start =
        cursor === undefined ? 0 : this.#index.outlineIds.indexOf(cursor) + 1;
      if (cursor !== undefined && start === 0)
        invalid("Outline list cursor is invalid.");
      ...
      return {
        outlines,
        ...(start + limit < this.#index.outlineIds.length
          ? { nextCursor: ids.at(-1)! }
          : {}),
      };
    });
