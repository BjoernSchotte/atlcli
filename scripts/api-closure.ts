...
export function closureForPackage(pkgDir: string, pkgName: string): EntrypointClosure[] {
  const entries = entryPointsOf(pkgDir);
  ...
  for (const entry of entries) {
    if (entrypointStability(pkgName, entry.label) !== "stable") continue;
    for (const name of exportsOf(entry.dtsPath).names) stableNames.add(name);
  }

  return entries.map((entry) => {
    const stability = entrypointStability(pkgName, entry.label);
...
export function closureReportFor(pkgDir: string, pkgName: string): string {
  ...
  const lines: string[] = [
    ...
    "",
    `> **Freeze decision (spec 009 T4.2): ${decision.frozen ? "1.0.0 — FROZEN" : "stays 0.x — NOT frozen"}.**`,
    `> ${decision.reasoning}`,
    ">",
