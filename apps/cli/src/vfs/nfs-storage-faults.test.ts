...
it.skipIf(process.env.ATLCLI_NFS_STORAGE_FAULTS !== "1")("recovers CREATE and UPDATE after real filesystem ENOSPC following remote success", async () => {
  ...
  try {
    ...
    // Reuse one owned volume; each operation has a separate journal and cache.
    for (const operation of ["create", "update"] as const) {
      try {
        ...
        const openVfs = () => ConfluenceVfsImpl.open({ profile: "fixture", client, spaces: ["DOCSY"],
          mode: "rw", allowDelete: false, coalesceMs: 0, cacheDir: join(root, operation, "cache"), offline: false });
        vfs = await openVfs();
        const journalPath = join(mount, operation + ".sqlite");
        ...
        console.info("[storage-fault] verified " + operation);
      } finally {
        await publisher?.stop(); publisher = undefined;
        journal?.close(); journal = undefined;
        await vfs?.close(); vfs = undefined;
