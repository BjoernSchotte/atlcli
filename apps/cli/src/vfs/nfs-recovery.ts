...
export function recoverNfsJournal(path: string, options: { id?: string; output?: string; image?: string } = {}) {
  ...
  try {
    ...
    if (schema !== 8 && schema !== 9) {
      throw new Error("Recovery requires NFS journal schema 8 or 9; keep the original journal and use its matching CLI version");
    }
    if (!options.id) {
      return db.query(`SELECT files.id,files.path,files.baseVersion,files.revision,publishedRevision,error,
        ...
        LEFT JOIN intents USING(id) ${schema === 9 ? "LEFT JOIN creations USING(id)" : ""} ORDER BY files.id`).all();
    }
