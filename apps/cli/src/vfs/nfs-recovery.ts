...
export function recoverNfsJournal(path: string, options: { id?: string; output?: string; image?: string } = {}) {
  ...
  try {
    ...
    if (db.query<{ user_version: number }, []>("PRAGMA user_version").get()?.user_version !== 8) {
      throw new Error("Recovery requires NFS journal schema 8; keep the original journal and use its matching CLI version");
    }
    if (!options.id) {
      return db.query(`SELECT files.id,files.path,files.baseVersion,files.revision,publishedRevision,error,
        ...
        LEFT JOIN intents USING(id) ORDER BY files.id`).all();
    }
