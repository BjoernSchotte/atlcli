...
export async function reconcileNfsMove(vfs: ConfluenceVfs, journal: NfsJournal, source: string): Promise<boolean> {
  ...
  const confirmed = () => vfs.confirmMove(move.id, move.spaceKey, move.targetParentId, move.title, move.kind);
  ...
  if (!await vfs.confirmMove(move.id, move.spaceKey, move.targetParentId, move.sourceTitle, move.kind)) return false;
  const intermediate = posix.join(posix.dirname(move.target), posix.basename(move.source));
  await vfs.rename(intermediate, move.target, { id: move.id, spaceKey: move.spaceKey,
