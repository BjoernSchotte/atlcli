...
import { mountCommandFor, unmountCommandFor } from "../commands/wiki-mount.js";
...
beforeAll(async () => {
  ...
  if (KERNEL) {
    ...
    if ("run" in attach) spawnSync(attach.run[0]!, attach.run.slice(1), { stdio: "inherit" });
  }
});
...
describe.skipIf(!KERNEL).serial("wiki mount through the macOS kernel client", () => {
  it("lists the volume through the real filesystem", () => {
    const entries = readdirSync(mountpoint);
    expect(entries).toContain(E2E_SPACE_KEY);
  });

  it("reads a page through the real filesystem", () => {
    const spaceDir = join(mountpoint, E2E_SPACE_KEY);
    const first = readdirSync(spaceDir).find((name) => /-\d+$/.test(name));
    expect(first).toBeDefined();
    const text = readFileSync(join(spaceDir, first!, "_index.md"), "utf8");
   ...
   */
  it("exposes the Spotlight exclusion at the volume root", () => {
    expect(readdirSync(mountpoint)).toContain(".metadata_never_index");
    expect(readFileSync(join(mountpoint, ".metadata_never_index"), "utf8")).toBe("");
  });
});
