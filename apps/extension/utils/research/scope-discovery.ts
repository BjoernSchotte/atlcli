...
export type ResearchScopeSourceV1 =
  ...
  | "exact_link"
  | "research_discovery";
export type ResearchScopeAuthorityV1 =
  | "candidate"
  | "resolved"
  | "approved"
  | "locked";
...
function mentionNeedle(mention: ResearchScopeMentionV1): string {
  return mention.normalizedText.trim() || normalize(mention.text);
}

function candidateMatches(
  mention: ResearchScopeMentionV1,
  candidate: ResearchScopeCandidateV1
): "exact_key" | "exact_name" | "alias" | "prefix" | "fuzzy" | undefined {
  const needle = mentionNeedle(mention);
  ...
  if (needle && name.split(" ").some((word) => needle.split(" ").includes(word))) {
    return "fuzzy";
  }
  return undefined;
}
...
export function resolveResearchScopeMentionV1(
  input: ResearchScopeResolutionInputV1
  ...
  const available = input.candidates.filter(
    (candidate) =>
      candidate.tenantOrigin === input.expectedTenantOrigin &&
      candidate.accessible &&
      (!input.mention.productHint || candidate.product === input.mention.productHint) &&
      (!input.mention.entityKindHint || candidate.entityKind === input.mention.entityKindHint) &&
      (input.allowArchived || candidate.status !== "archived")
  );
  const ranked = available
    .map((candidate) => ({
      candidate,
      match: candidateMatches(input.mention, candidate),
    }))
    .filter((entry): entry is { candidate: ResearchScopeCandidateV1; match: NonNullable<ReturnType<typeof candidateMatches>> } => Boolean(entry.match))
    .sort((left, right) => {
      const rankDelta = MATCH_RANK[right.match] - MATCH_RANK[left.match];
  ...
  if (exactKey.length === 1) {
    return {
      ...
      state: "resolved",
      candidateIds,
      resolvedCandidateId: exactKey[0]!.candidate.id,
  ...
  if (exactName.length === 1 && input.catalogComplete) {
    return {
      mentionId: input.mention.id,
      state: "resolved",
      candidateIds,
      resolvedCandidateId: exactName[0]!.candidate.id,
      uniquenessProof: "complete_catalog",
      catalogComplete: true,
      requiresUserChoice: false,
    };
  }
  if (exactName.length > 1 || ranked.length > 0) {
    return {
      mentionId: input.mention.id,
      state: input.catalogComplete ? "ambiguous" : "incomplete",
      candidateIds,
      catalogComplete: input.catalogComplete,
  ...
  return {
    mentionId: input.mention.id,
    state: input.catalogComplete ? "not_found" : "incomplete",
    candidateIds: [],
    catalogComplete: input.catalogComplete,
    requiresUserChoice: !input.catalogComplete,
  };
}

export function createResearchScopeBindingV1(input: {
  candidate: ResearchScopeCandidateV1;
  source: ResearchScopeSourceV1;
  authority: Extract<ResearchScopeAuthorityV1, "approved" | "locked">;
  mentionId?: string;
  approvedAt?: string;
  bindingId?: string;
}): ResearchScopeBindingV1 {
  return {
    id: input.bindingId ?? `scope-binding:${input.candidate.id}`,
    tenantOrigin: input.candidate.tenantOrigin,
    product: input.candidate.product,
    entityKind: input.candidate.entityKind,
    entityRef: input.candidate.entityRef,
    ...(input.candidate.key ? { key: input.candidate.key } : {}),
...
export function selectResearchScopeSeedsV1(
...
export function projectApprovedWholeScopeV1(
  bindings: readonly ResearchScopeBindingV1[],
  base: ResearchScopeV1
): ResearchScopeV1 {
  ...
  for (const binding of bindings) {
    ...
    if (binding.entityKind === "project" && binding.product === "jira" && binding.key) {
      jiraProjectKeys.add(binding.key.toUpperCase());
    }
    if (binding.entityKind === "space" && binding.product === "confluence" && binding.key) {
      confluenceSpaceKeys.add(binding.key);
    }
  }
  return {
    ...base,
    jiraProjectKeys: [...jiraProjectKeys],
    confluenceSpaceKeys: [...confluenceSpaceKeys],
  };
}
